...
+ the *recursion rule* (4) where we repeatedly define the function in
  terms of the previous application of the function.
...
\begin{align*}
g(4) &= \bbox[yellow,3px]{g(3)} + 4 \\
     &= \bbox[yellow,3px]{(g(2) + 3)} + 4 \quad\quad \text{...substituting previous} \\
     &= ((\bbox[lightgray,3px]{g(1)} + 2) + 3) + 4 \quad\quad \text{...substituting previous} \\
     &= \bbox[lightgray,3px]{1} + 2 + 3 + 4 \quad \text{...finally substituting in base case}\\
     &= 10
\end{align*}
⌟
...
or a /dotted pair/ since we have just a single cons cell with ~b~ in
the ~car~ compartment and ~c~ in the ~cdr~ compartment---with nothing
further linked. And so, to have a real list the final /cdr/
compartment of the last cons cell must point to, be consed with the
~nil~ or the /empty set/. By this definition ~(cons 1 '())~ is a
