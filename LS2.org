...
repeatedly. $\sum_{i=1}^n i$ can be redone as a two-part algebraic function
...
Now we nest these intermediary steps[fn:18]

\begin{align*}
g(4) &= g(3) + 4 \\
     &= (g(2) + 3) + 4 \quad\quad \text{...substituting previous} \\
     &= ((g(1) + 2) + 3) + 4 \quad\quad \text{...substituting previous} \\
     &= 1 + 2 + 3 + 4 \\
     &= 10
\end{align*}
⌟
...
Scheme has a function *list* that creates a list, i.e., conses
together sexp's
...
But as we've seen, this list ~(1 2 3 4)~ is really this
...
So the build-in Scheme function ~list~ is somehow creating the above
sexp. Let's write our own (more basic) version of a list-making
function that conses together things, primarily to see what role
recursion plays
...
(define (makelist n)
  (if (= n 1)
      (list 1)                       ; base case. Return (1) if true
...
We ~define~ the function as ~makelist~ and set one argument ~n~ for
...
~(list 1)~ creates ~(1)~. Otherwise, the next line ~(cons n (makelist
(- n 1)))~ is called. Got that? Not quite?

Two big things are going on in this short piece of code. First, we
have the *conditional* where a decision is made

#+name: 91f7c57c-4c1d-46be-8a03-fa9f35fb3217
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(define (myvariable n) (if (> n 12) 1 0))
#+end_src
...
[fn:17] /...until true/ belies a test for true or false, doesn't it?
Hold that thought...

[fn:18] This math nesting in parentheses is just like nesting in
