...
with lists. Consider this simple summation of the natural numbers
$\mathbb{N}$
...
This sums the sequence[fn:16] of the counting numbers up to and
including the nth element of $\mathbb{N}$. But we can also sum any
$\mathbb{N}$ sequence /recursively/ by defining a system that will
progress to the answer by referring or calling itself
repeatedly. $\sum_{i=1}^n i$ can be redone as a two-part algebraic
function[fn:17]
...
This *[[https://en.wikipedia.org/wiki/Piecewise_function][piecewise function]]* approach to finding the sum has what is
called the *base case* (3), which is also the terminal case, i.e., if
$n = 1$ we are done and the answer given is simply $1$. But if $n \gt 1$
then we apply the *recursion rule* (4) where we repeatedly define the
function in terms of previous application of the function. We keep
stepping backward until we arrive at the base or termination case. Got
that? Better to just try it out
...
Now we nest these intermediary steps[fn:19]
...
We ~define~ the function as ~makelist~ and give one argument ~n~ for
user input. Next, we must test what the incoming input is. If the
incoming value is ~1~ then the test expression ~(= n 1)~ is true and
~(list 1)~ creates the list ~(1)~. Otherwise, the next line ~(cons n
(makelist (- n 1)))~ is called. Got that? Not quite?

Two big things are going on in this short piece of code. First, we
have the *[[https://en.wikipedia.org/wiki/Conditional_(computer_programming)][conditional]]* where a decision is made which part of the
recursion function we need to apply, 1) the base case or, 2) the
recursion rule. Below is a simpler /one-liner/ that makes it easier to
see what's going on. ~myvariable~ takes in an argument and "decides"
that if it greater than ~12~ the output is ~1~, but if it is less than
or equal to ~12~ it returns ~0~
...
So in ~mytest~ the test $t_1$ is the sexp ~(> n 12)~ and $sexp_1$ is
~1~ and $sexp_2$ is ~0~. As you can probably surmise, we're asking /if
the incoming ~n~ is greater than ~12~ then return ~1~, if not default
to ~0~.
...
itself/, and it is passing an argument decremented, ~1~
less.@@html:</font>@@
...
return ~(1)~ giving us the desired
...
Again, we'll go into detail on conditionals and especially all the
lore on recursion soon. But for now let's look at what a list is
"under the hood," which shows us simultaneously how the individual
elements can be manipulated with the Scheme primitives ~car~ and
~cdr~.
\\
\\
