...
with lists. Consider this simple summation of a *sequence* of natural
numbers $\mathbb{N}$
...
This sums the sequence of the counting numbers up to and including
$n$. We can also write a formula for summing any $\mathbb{N}$ sequence
/recursively/ by defining a system that will march, progress to the
answer by referring or calling itself repeatedly. So $\sum_{i=1}^n i$ can be
redone as a two-part algebraic function
...
called the *base case* (3), which is also the terminal case, i.e.,
when $n = 1$ we are done and the answer is simply given as $1$. But if
$n \gt 1$ then we apply the *recursion rule* (4) where we define, we
establish the function application at hand in terms of previous or
next application of the function, or, one step in terms of the
previous (or next) step ... which we then repeat stepping backward (or
forward) until we arrive at the base or termination case. Got that?
Better to just try it out once

⌜ \\
𝖟𝕭: If $n = 4$ then we start with the recursion rule (4) until the
base case is /true/[fn:16]
...
Now we nest these intermediary steps[fn:17]
...
Have we seen this step-wise nesting before? Yes, when we saw how lists
are really just the consing of objects together. Scheme has a function *list*
...
[fn:16] /...until true/ belies a test for true or false, doesn't it?
Hold that thought...

[fn:17] This nesting in parentheses we see above is just like nesting
in parentheses in Scheme of course. Parentheses keep law and order.
