...
evaluated, produces output[fn:10]
...
This expression will sum the sequence[fn:16] of the counting numbers
...
$\sum_{i=1}^n i$ as a two-part algebraic function[fn:17]
...
Got that? Still too abstract?[fn:18] Right. Example time...
...
Now we nest these intermediary steps[fn:20]
...
how Scheme does basic recursion.[fn:21]
...
apply, 1) the base case or, 2) the recursion rule. Below is a simpler
/one-liner/ that makes it easier to see what's going on. ~mytest~
takes in an argument and "decides" that if it greater than ~12~ the
output is ~1~, but if it is less than or equal to ~12~ it returns ~0~
...
[fn:18] Often in math you'll read the abstract, general rule, then
...
[fn:19] /...until true/ seems to suggest some test for true or false,
doesn't it?  Hold that thought...

[fn:20] This math nesting in parentheses is just like nesting in
...
[fn:21]  Note: code line numbers are not part of the Scheme code.
