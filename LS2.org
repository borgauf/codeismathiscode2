...
forty thousand years. Others say sixty to one-hundred thousand
years. And yet we surely have been anatomically /Homo sapiens/ longer
than we've had language. What did we, with our big brains, do before
we had language?[fn:10] Obviously, we relied to some extent on
intuitions, i.e., we would see two piles of nuts and only have vague
intuitive notions that there were two piles and that they might differ
in size... But if we just analyze that last sentence we can begin to
...
feed the session a symbol[fn:12]
...
evaluated, produces output[fn:13]
...
been nudging around the idea of how we build working code from
lists. Now we'll explore more deeply what a list as an sexp really
is. We'll start with a more formal look at s-expressions.
...
...which is, er, somewhat abstract. But didn't we just say a Scheme
program is completely contained in lists, data and code? We've seen
the atom before. Recall, an atom is just a symbol object thing
standing alone by itself. In the Scheme world, atoms can do two
...
far. For example, we saw earlier how we could put ~'myatom~ in the
REPL---and we only got ~myatom~ repeated back to us, i.e., no real
computation is happening yet.
...
Scheme just gave us back a cons cell. Let's play with this idea
...
So far we've exampled our way into how lists are built in Scheme,
...
inside another. And yet both the inner and outer ~cons~ /consed/ the
sexp's into a real list, making the ~cons~ dots go away. Why and how?
...
So ~(1 2 3)~ is really just a visually improved version of ~(1 . (2
. (3 . '())))~.[fn:14] Awkward looking as the naked version is, it shows the
details of how we're doing a particular sort of /connecting/ or
/associating/ ~1~, ~2~, and ~3~ together. But it's probably not
apparent why we need to *cons*-truct a list this way. Let's take a
short "math holiday" to get started.

⌜\\
@@html:<font color = "#0d3db3">@@⥤ *Math Holiday! ... working binarily* @@html:</font>@@[fn:15]
...
mathematical abstraction), we would see a stack of numbers like this
...
when we tackle a stack of numbers like above we are actually keeping a
running tab in our heads of adding /only two of the numbers at a
time/, i.e., /two/ inputs produce /one/ output. So let's start at the
top with $1 + 2$, which is $3$. We then "carry along" this temporary
result and add the next number $3$ to it, and so on. In reality after
that first binary operation of $1 + 2$, we consider the result $3$ to
be the *augend*, while the next number $3$ becomes the *addend*. The
intermediary total of this sum then becomes the next *augend*, and so
we progress down the stack reassigning the latest sum to the *augend*
and adding the next *addend* to it. This is how many digital logic
calculations are done. Lots more later. \\
⌟

Now with a refreshing math holiday under our belts it should be clear
that ~cons~ as a binary operation must operate similarly, i.e., there
is no such thing as ~(cons 1 2 3)~,[fn:16] rather, ~cons~ is a /binary
...
𝖟𝕭: When we see ~(cons 1 (cons 2 (cons 3 '())))~ we're seeing the
initial binary operation of ~(cons 3 '())~ which is ~(3)~. This is
embedded in ~(cons 2 (3))~ which evaluates to ~(2 3)~, which is again
embedded in ~(cons 1 (2 3))~ resulting in ~(1 2
3)~. $\mathfrak{Fazit}$: ~cons~ is always consing just two sexp's.

It may seem too soon to go on holidays again, but let's do another
math holiday anyway!

⌜\\
@@html:<font color = "#0d3db3">@@⥤ *Math Holiday! ... intro to
recursion* @@html:</font>@@

*[[https://en.wikipedia.org/wiki/Recursion][Recursion]]* is a central idea in math and CS. Let's take a quick look
at it in a basic theory way before we actually start practicing it
with Scheme code. Consider this simple summation of the natural
numbers $\mathbb{N}$
...
We can express this formula for summing the natural numbers
/recursively/
...
[fn:10] ...some suggest telepathy...

[fn:11] So is Professor Smullyan saying ignorance is bliss?

[fn:12] From now on for visual purposes we will forego the code block
...
[fn:13] Anything following a ";" is a /comment/ for the rest of that
...
[fn:14] Realize of course that \\
~(1 . (2 . (3 . NIL)))~ \\
is just a symbolic way of expressing \\
~(cons 1 (cons 2 (cons 3 '())))~. \\
The /dotted/ version doesn't work in actual Scheme coding.

[fn:15] [[https://en.wikipedia.org/wiki/Binary_operation][This Wikipedia article on binary operations]] is quite
...
[fn:16] Most Scheme REPLs will give the error ~wrong number of
