...
**procedural** or **imperative** programming, emphasizes **functions**
as the computational building blocks. Key is how functions in
functional programming languages behave as if they were true
mathematical functions. This is a great advantage---for reasons we
...
set a /class/, by the way.)}}}  ⇲@@html:<font color = "#0d3db3">@@ A
**function** is a **relation** between a **set** whose elements are
inputs (the **domain**) and a set whose elements make up outputs (the
**range**) with the key property that /each input is related to
exactly one output/...and no more than one
...
simplified to find equivalent values. Repeating, @@html:<font color =
"#0d3db3">@@expressions have values.@@html:</font>@@ The expression $8
\cdot 2 - 5$ simplifies to $16 - 5$ and then evaluates to $11$. And
sometimes a mathematical expression comes from transcribing a
statement, e.g., "One number is six more than half of another number."
So if we represent the "other number" as $a$ then a mathematical
expression of this statement could be
...
\\
⇲@@html:<font color = "#0d3db3">@@ An *expression* is a mathematical
...
𝖟𝕭: $y = e^x$ is an expression because, first and foremost, it is
...
𝖟𝕭: $7a + 2b + 3c$ is clearly an /algebraic/ expression but it does
...
on with this function-expression thingy. *[[https://en.wikipedia.org/wiki/Currying][currying]]* will be discussed. \\
\\
...
that this is a list? \\
~(atom turkey) or~ \\
@@html:<span class="schemer">@@Λιττλε Σξηεμερ:@@html:</span>@@ No,
...
So it's obvious that Scheme is organized into sexp's, and from before
we established that expressions have values. But why, how? \\
\\
...
feed the session a symbol[fn:9]
...
whole piece of real code *A* as just inert /input/ to another piece of
code *B*. We'll explore functions that take functions as input soon,
...
on: ~(function sexp1 sexp2 ... sexpN)~ or ~(operator operand1 operand2
...)~ @@html:</font>@@ \\
\\
...
function/operator. In keeping with this rule of operator first, we see
even the arithmetic operators like ~+~, ~*~, et cetera, using [[https://en.wikipedia.org/wiki/Polish_notation][prefix
notation]] and yes, there can in the case of addition and multiplication
be multiple arguments or operands. Now, let's see a full Scheme
function that takes one argument and when evaluated, produces
output[fn:10]
...
#+begin_figure
#+CAPTION: Syntax tree and linked list for ~a + 7 * x~ 
[[file:images/SyntaxTree.png]]
#+end_figure

Jumping ahead, we see an expression, its representation as an
*[[https://en.wikipedia.org/wiki/Abstract_syntax_tree][abstract symbol tree]]* and its representation as a linked list. The
expression in Scheme syntax would be

#+name: 581f18ff-e72d-4694-8d2d-b0974966a42b
#+begin_src scheme :eval never :exports code
(+ a (* 7 x))
#+end_src

This shows the fact that Lisp/Scheme programs built from lists are
already their own 
...
613 '()))~ or ~(42 . (69 . (613 . NIL)))~ we see the three individual
...
and a DIY ascii representation would be
...
on a mainframe. But they are also primitive operators like ~cons~. So
if ~(cons 'b 'c)~ produces ~(b . c)~ then
...
Did this surprised you? Again, ~car~ returns the first element of the
list aka the *head* of the list; and ~cdr~ returns the *tail* or
the /rest of the list/. Let's look at ~cdr~ without syntactic sugar
...
As we see with the result, Scheme throws back in the sugar. The tail
of ~(cons 42 (cons 69 (cons 613 '())))~ was returned, i.e., ~(cons 69
(cons 613 '()))~ $\rightarrow$ ~(69 613)~. Let's now reverse this
...
Now, is ~(b . c)~ a proper list? No, it is called an /improper/ list
or a /dotted pair/ since we have ~b~ in the ~car~ compartment and ~c~
in the ~cdr~ compartment. To have a real list the final /cdr/
compartment of the last cons cell must point to, be consed with the
~nil~ or the /empty list/. By this definition ~(cons 1 '())~ is a
proper list, but something like ~(a b c . d)~ has as the last /cdr/
pointing to ~d~ which is not ~nil~.
...
Scheme gives us back a cons cell.[fn:11] Let's play with this idea
...
(cons 2 (cons 3 '())))~.[fn:12] Awkward looking as the naked version
...
⌜\\
@@html:<font color = "#0d3db3">@@⥤ *Math Holiday! ... working binarily* @@html:</font>@@[fn:13]
...
is no such thing as ~(cons 1 2 3)~,[fn:14] rather, ~cons~ is a /binary
...
3 '())))~, we're seeing precedence-wise[fn:15] the initial /binary/
...
** Lists are the shipping containers of the Scheme world

Was it a bit weird to see

#+name: 3f35efa9-0c5b-4d80-add3-33098746a6a0
#+begin_src scheme :eval never :exports code
(((five is a nice number) 5) but so is six)
#+end_src

i.e., all that stuff crammed willy-nilly between
parentheses?  We seem to be building a real computer language out of
surrounding things in parentheses. As we've discussed, Scheme's basic
syntax is ~(operator operand1 operand2 ...)~, but what would happen if
we put ~(((five is a nice number) 5) but so is six)~ into the Guile
REPL and hit Enter?

#+name: e4cf4dfb-5472-4f48-a6b7-6a316e808469
#+begin_src scheme :eval never :exports code
(((five is a nice number) 5) but so is six)
#+end_src

#+begin_example
#+RESULTS: fa92687e-212c-471e-8090-ed06a91e3b8d
: ice-9/boot-9.scm:1676:22: In procedure raise-exception:
;;; <stdin>:446:3: warning: possibly unbound variable `five'
;;; <stdin>:446:8: warning: possibly unbound variable `is'
;;; <stdin>:446:11: warning: possibly unbound variable `a'
;;; <stdin>:446:18: warning: possibly unbound variable `number'
;;; <stdin>:446:29: warning: possibly unbound variable `but'
;;; <stdin>:446:33: warning: possibly unbound variable `so'
;;; <stdin>:446:39: warning: possibly unbound variable `six'
#+end_example

Looks like quite the mess---all because that first sexp ~((five is a
nice number) 5)~ is /not an operator/. Let's try again, this time
quoting the whole sexp

#+name: 86e22bbd-e740-447d-9cbb-dadbbc0dba53
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
'(((five is a nice number) 5) but so is six)
#+end_src

#+RESULTS: 86e22bbd-e740-447d-9cbb-dadbbc0dba53
: (((five is a nice number) 5) but so is six)

Recall, in the functional world of Scheme an sexp /has a value/, i.e.,
can be evaluated. And so the non-quoted version could not be
successfully evaluated, while the quoted evaluated to itself. Still,
lists may contain whatever we like. They can be as big and long and
complex as we like---as long as we handle them correctly. Lists in the
Lisp-Scheme family are truly the shipping containers of both code and
data. Consider this Emacs Lisp list

#+name: a4fb25c4-16ea-4df7-80d5-057e0dec0e66
#+begin_src scheme :eval never :exports code
(custom-set-variables
 '(TeX-electric-math '("$" . "$"))
 '(add-org-graph-puhc nil)
 '(agda2-program-name "/home/myhome/.local/bin/agda")
 '(column-number-mode t)
 '(company-global-modes '(not org-mode))
 '(coq-prog-args
   '("-topfile" "/home/myhome/Dropbox/org/coq/math-comp-test/mct1.v" "-emacs"))
 '(display-battery-mode t)
 '(display-time-mode t)
 '(doc-view-continuous t)
 '(ediprolog-program "/home/myhome/bin/swipl")
 '(ediprolog-system 'swi)
 '(geiser-active-implementations '(mit chicken chez guile))
 ...
 ...
 '(sml-program-name "/home/myhome/opt/smlnj110.99.4/bin/sml")
 '(tool-bar-mode nil)
 '(user-full-name "Little Schemer")
 '(warning-suppress-types '((org) (comp))))
#+end_src

i.e., one single list stretches over 150 lines---all of that just
one list! Again, looking in the first position we find the Elisp
function ~custom-set-variables~, which according to the Emacs Help
will
...
where the quoted arguments following ~custom-set-variables~ are the
various customization variables Emacs will use to set up a working
environment upon startup.

Lisp before Scheme had a rich history in such disparate uses of the
basic list for data representation. Much of that carries on. For
example, a Scheme 

Here's a truly wild-looking use of a list where an *[[https://en.wikipedia.org/wiki/Abstract_syntax_tree][abstract syntax
tree]]* is created from an org-mode file. Here is the simple org-mode
file
...
on which we'll use Org Element's command ~org-element-parse-buffer~,
which will *[[https://en.wikipedia.org/wiki/Parsing][parse]]* that seven-line org-mode file into an /abstract
syntax tree/, all of it packed into single huge list

#+begin_src emacs-lisp :eval never :results code replace
...
(org-data
 (:standard-properties
  ...
  :path "/home/myhome/Dropbox/org/codeismathiscode2/ElementTest.org" :CATEGORY "ElementTest")
 (section
  (:standard-properties
...
Yes, what you see above is one single very huge list with lots of
nested lists, all of it representing structured data, although we have
cut out lots (note the ... gaps) to fit it on the page. (Also, use the
slider to see the hidden parts.) As mentioned above, this is a list
acting as an /abstract syntax tree/.[fn:16] ASTs are central to
programming in that any programming code must be parsed, i.e.,
transformed into an AST in order to be evaluated by the interpreter or
compiler.

** Recursion, let's get started with...

*[[https://en.wikipedia.org/wiki/Recursion][Recursion]]* is a central idea in math and CS. Let's take a quick
theoretical look and then draw some parallels to what we've been doing
with lists. We'll go much deeper into the whole world of recursion as
we move along.

Consider this simple summation of the natural numbers $\mathbb{N}$

\begin{align*}
\sum_{i=1}^n i = 1 + 2 + 3 + \ldots + (n - 1) + n
\end{align*}

This expression will sum the sequence[fn:17] of the counting numbers
up to and including the nth element of $\mathbb{N}$. But let's look at
a different way to sum any $\mathbb{N}$ sequence, namely,
/recursively/ by setting up a method that progresses to the answer
stepwise by referring or calling itself repeatedly. Let's reimagine
$\sum_{i=1}^n i$ as a two-part algebraic function[fn:18]

\begin{align}
g(1) &= 1 \\
g(n) &= g(n-1) + n,\; \text{for all} \; n \geq 2
\end{align}

This *[[https://en.wikipedia.org/wiki/Piecewise_function][piecewise function]]* made up of (3) and (4) breaks the problem
into two parts:

+ the *base case* (3), which is also the terminal case, i.e., if $n =
  1$ we are done and the answer given is simply $1$, and
+ the *recursion rule* (or iteration rule) (4) where we repeatedly
  define the function in terms of the previous application of the
  function.

Got that? Still too abstract?[fn:19] Right. Example time...

⌜ \\
𝖟𝕭: What is the sum of the $\mathbb{N}$ sequence ending with $n = 4$?
\\

If $n = 4$ then no, this is obviously not the base-terminal case $n =
1$, so we start applying the recursion rule (4) until the base case is
in fact /true/[fn:20]

\begin{align*}
g(4) &= g(4 - 1) + 4 = g(3) + 4 \quad\quad \text{...apply rule} \\
g(3) &= g(3 - 1) + 3 = g(2) + 3 \quad\quad \text{...apply rule} \\
g(2) &= g(2 - 1) + 2 = g(1) + 2 \quad\quad \text{...apply rule} \\
g(1) &= 1 \quad\quad \text{...apply base case} 
\end{align*}

Now we nest these intermediary steps[fn:21]

\begin{align*}
g(4) &= \bbox[yellow,4px]{g(3)} + 4 \\
     &= \bbox[yellow,4px]{(g(2) + 3)} + 4 \quad\quad \text{...substituting previous} \\
     &= ((\bbox[lightgray,4px]{g(1)} + 2) + 3) + 4 \quad\quad \text{...substituting previous} \\
     &= \bbox[lightgray,4px]{1} + 2 + 3 + 4 \quad \text{...finally substituting in base case}\\
     &= 10
\end{align*}
⌟

So a journey of a thousand miles begins with a single step---followed
by another and another and another. Or, How do you descend a flight of
stairs? You take one step down from the top step, then repeat, until
you reach the bottom base case with no more steps to take. But what
tells you to keep stepping until the end is reached? We'll discuss
that now.

Scheme has a built-in function ~list~ that creates a list, i.e.,
conses together sexp's

#+name: e89ee17e-79cd-4ed6-9490-a09665859bcc
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(list 1 2 3 4)
#+end_src

#+RESULTS: e89ee17e-79cd-4ed6-9490-a09665859bcc
: (1 2 3 4)

But as we've seen, a list such as ~(1 2 3 4)~ is really

#+name: 7ec3afa7-8c6e-46ee-b734-fab14a8eef95
#+begin_src scheme :eval never :exports code
(cons 1 (cons 2 (cons 3 (cons 4 '()))))
#+end_src

So ~list~ conses together all the arguments it is given. Let's write
our own (more basic) version of a list-making function that conses
together just integers, primarily to see what role recursion plays and
how Scheme does basic recursion.[fn:22]

#+name: 3860bfc7-d02e-4ff8-90fa-0a37c26df4db
#+begin_src scheme +n :session *littleschemer* :results verbatim :exports code 
(define (makelist n)
  (if (= n 1)
      (list 1)   ; base case. Return the list (1) if true
      (cons n (makelist (- n 1)))))  ; else recursive case.
#+end_src

Testing it out, we'll make a list of four elements

#+name: 7b92bc88-8e01-4cdd-821e-0e28f09efd38
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(makelist 4)
#+end_src

#+RESULTS: 7b92bc88-8e01-4cdd-821e-0e28f09efd38
: (4 3 2 1)

On line 1 we ~define~ the function as ~makelist~ and give one argument
~n~ for user input. On lines 2 through 4 we have the greater
*if-then-else* sexp which starts on line 2 with the sexp ~(= n 1)~
("is ~n~ equal to ~1~?") as the test. If the incoming value is ~1~
then the test expression ~(= n 1)~ is true and on line 3 ~(list 1)~
creates the list ~(1)~. Otherwise, on line 4 ~(cons n (makelist (- n
1)))~ is called. Got that? Not quite?

Two big things are going on in this short piece of code. First, we
have the *[[https://en.wikipedia.org/wiki/Conditional_(computer_programming)][conditional]]*, the commonly called /if-then-else/ where a
decision is made on which part of the recursion function we need to
apply, 1) the base case or, 2) the recursion/iteration rule. Below is
a simpler /one-liner/ that makes it easier to see what's going
on. ~mytest~ takes in an argument and "decides" that if it greater
than ~12~ the output is ~1~, but if it is less than or equal to ~12~
it returns ~0~

#+name: 91f7c57c-4c1d-46be-8a03-fa9f35fb3217
#+begin_src scheme :session *littleschemer* :results verbatim :exports code
(define (mytest n) (if (> n 12) 1 0))
#+end_src

#+RESULTS: 91f7c57c-4c1d-46be-8a03-fa9f35fb3217
: #<unspecified>

Let's test it

#+name: 736dc827-2564-49dc-ba1c-7c57f88f5933
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(mytest 15)
#+end_src

#+RESULTS: 736dc827-2564-49dc-ba1c-7c57f88f5933
: 1

The basic form of a conditional test is \\
\\
@@html:<font color = "#0d3db3" size = 5%>@@ $\quad\quad\quad\quad$ if $t_1$ then $sexp_1$ else $sexp_2$
@@html:</font>@@ \\

In ~mytest~ the test $t_1$ is the sexp ~(> n 12)~ and $sexp_1$ is ~1~
and $sexp_2$ is ~0~. As you can probably tell, we're asking /if the
incoming ~n~ is greater than ~12~ then return ~1~, if not default to
~0~/.

Next chapter we'll take a much closer look and really get into the
weeds of the whole /conditional/, if-then-else story, but for now
let's get back to our recursion example. So in

#+name: 4c9bad9e-b349-4406-9033-85b71b441593
#+begin_src scheme -n 1 :eval never :exports code
(define (makelist n)
  (if (= n 1)
      (list 1)   ; base case. Return the list (1) if true
      (cons n (makelist (- n 1)))))  ; else recursive case.
...
The beauty of this code is what happens if ~n~ does not equal ~1~. In
that case the incoming ~n~ value is consed to ... what's happening
there?! What is happening is recursion. @@html:<font color =
"#0d3db3">@@ The function ~makelist~ is quite literally /calling
itself/, and it is passing an argument decremented, i.e., ~1~
less.@@html:</font>@@

This can be hard to picture. Let's walk through it when ~n~ is
~4~. We'll use a built-in utility in Guile Scheme called ~trace~ at
the REPL input

#+begin_example
scheme@(guile-user)> ,trace (makelist 4)
trace: |  (makelist 4)
trace: |  |  (makelist 3)
trace: |  |  |  (makelist 2)
trace: |  |  |  |  (makelist 1)
...
This indicates how ~makelist~ is going down rabbit holes, i.e.,
~makelist~ calls itself repeatedly until the condition ~(= n 1)~ is
satisfied, then the terminating /base case/ ~(list 1)~ is called and
the program climbs back out of the rabbit holes. When we first call
~makelist~ with ~n~ as ~4~, it lands in the ~else~ sexp

#+name: 3a0c3790-0239-4c20-ab12-ef4898559302
#+begin_src scheme :eval never :exports code
(cons n (makelist (- n 1))) ;else branch of if=then-else
#+end_src

which sets up

#+name: 81c243ab-465b-491e-b1fc-c46a56fc6022
#+begin_src scheme :eval never :exports code
(cons 4 (makelist 3)) ;makelist calls itself from within itself!
#+end_src

which re-enters ~makelist~ with argument ~3~. Again, the test ~(= n 1)~
fails and we have stacked up

#+name: 598bb5f9-7376-486d-b4a5-8d07e03c6c63
#+begin_src scheme :eval never :exports code
(cons 4 (cons 3 (makelist 2))) ;...calls itself again!
#+end_src

then

#+name: ad58c5ba-d370-449e-8545-eb8914563e57
#+begin_src scheme :eval never :exports code
(cons 4 (cons 3 (cons 2 (makelist 1)))) ;calls itself again
#+end_src

Now when ~(makelist 1)~ is called it will pass the ~(= n 1)~ test and
return ~(list 1)~ giving us the desired

#+name: 4a6a0a68-a6d7-4e4a-be1f-2ea24d570295
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(cons 4 (cons 3 (cons 2 (list 1))))
#+end_src

#+RESULTS: 4a6a0a68-a6d7-4e4a-be1f-2ea24d570295
: (4 3 2 1)

And so this is the magic of recursion. Again, we'll go into detail on
conditionals and especially all the lore on recursion soon. But for
now let's look at what a list is "under the hood," which shows us
simultaneously how the individual elements can be manipulated with the
Scheme primitives ~car~ and ~cdr~.  
...
[fn:16] Soon we will explore how lists can represent sets, vectors,
matrices, graphs, and other structures.

[fn:17] For now we'll stick to the layman's use of the term
/sequence/. Later, we'll dive into what the math world thinks a
sequence is.

[fn:18] Recursion and [[https://en.wikipedia.org/wiki/Mathematical_induction][induction]] are closely related. We'll look into
induction later.

[fn:19] Often in math you'll read the abstract, general rule, then
explore examples, then say "A ha! Now I get it!" So yes, we will come
to love these awfully painfully formalistic, highly abstract
generalizations...

[fn:20] /...until true/ seems to suggest some test for true or false,
doesn't it?  Hold that thought...

[fn:21] This math nesting in parentheses is just like nesting in
parentheses in Scheme. Parentheses keep computational law and
order. The yellow and gray highlighting emphasizes the substitution.

[fn:22]  Note: code line numbers are not part of the Scheme code.
