...
Scheme gives us back a cons cell.[fn:14] Let's play with this idea
...
(cons 2 (cons 3 '())))~.[fn:15] Awkward looking as the naked version
...
⌜\\
@@html:<font color = "#0d3db3">@@⥤ *Math Holiday! ... working binarily* @@html:</font>@@[fn:16]
...
is no such thing as ~(cons 1 2 3)~,[fn:17] rather, ~cons~ is a /binary
...
3 '())))~, we're seeing precedence-wise[fn:18] the initial /binary/
...
[fn:14] This is also referred to as /dotted pair/.

[fn:15] Realize of course that \\
~(1 . (2 . (3 . NIL)))~ \\
is just the dotted symbolic way of expressing \\
~(cons 1 (cons 2 (cons 3 '())))~. \\
The dotted version doesn't work as actual Scheme code.

[fn:16] [[https://en.wikipedia.org/wiki/Binary_operation][This Wikipedia article on binary operations]] is quite
...
[fn:17] Some Scheme REPLs will give the error ~wrong number of
arguments~ because it was expecting exactly two inputs.

[fn:18] *precedence* is the order in which a simplification
