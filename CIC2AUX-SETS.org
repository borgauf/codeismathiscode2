...
#+EXPORT_FILE_NAME: ComputingInContext2AuxSets.html
...
/[[https://en.wikipedia.org/wiki/Set_theory][Set theory]]/ is the fundamental starting point of higher math, as well
...
to base itself on such a seemingly unassuming thing as a set. But as
...
So many sources start out with this deceptively simple and informal
...
The founder of set theory, Georg Cantor, back in 1874 defined a set
with his **Axiom of Unrestricted Comprehension**
...
And so Cantor means to[fn:2]

- establish some membership criteria,
- gather things passing this membership,
- and call it a set.
 
In math notation we can express this as

\begin{align*}
\exists S\, \forall x\, (x \in S \iff P(x))
\end{align*}

Reading this out loud, /there exists ($\exists$) a set $S$, and for all
($\forall$) $x$'s a particular $x$ is an element, a member of set $S$ if and
only if that $x$ satisfies the property $P$/. It says that if we have
a reasonably descriptive, defining property or criteria, actual things
passing this criteria can be grouped together and called a set. As it
turned out, Cantor was too freewheeling with his grasp of sets. His
definition allows us to create sets out of the absolute void---just
state a qualifying property, no matter how crazy---and everything in
the conceivable universe with that property exists as a legitimate
set. Trouble started when a famous logician Bertrand Russell, using
this loose definition, came up with a devastating paradox. We'll go
into **[[https://en.wikipedia.org/wiki/Russell%27s_paradox][Russell's Paradox]]** later, but suffice it to say the math world
was shocked---and had to go back to the drawing board on many things
long taken for granted. Whole schools of logic came crashing down. But
