...
The founder of set theory, Georg Cantor, defined a set with his
...
($\forall$) $x$'s a particular $x$ is an element of set $S$ if and only if
that $x$ satisfies the property $P$/. But before we go on, let's get a
better handle on just what's meant when we say /property/,
...
as ~parent(kathy)~. This is now considered a fact, a /statement/ ---
that is either true or false. ~parent~ itself is a /property/, i.e.,
the property of being a parent. In the form ~parent(X)~ with variable
~X~ it is now considered a /predicate/. The predicate ~parent(X)~ is
not yet true or false until we provide a value for ~X~, then it
becomes a statement. Bringing this into our discussion of sets, the
set of ~parent~'s can be expressed as[fn:3]
...
preliminaries. Hint: it's a very theoretical mathematical use of the
word.
