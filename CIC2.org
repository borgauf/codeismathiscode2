...
** Sets are the starting line of mathematics

**[[https://en.wikipedia.org/wiki/Set_theory][Set theory]]** is the generally accepted starting point of higher math, as well
as appearing early in most Discrete Math texts.[fn:1] It may strike
you as odd or ironic that the entire world of mathematics has decided
to base itself on such a seemingly unassuming thing as sets. But as
we gain more knowledge of set theory, the logical entailment will only
grow and the choice of set theory as a basis will make more and more
sense.

So many sources, especially treatments for the layperson, start out
set theory with this deceptively simple and informal statement, or
...
That seems even lighter and airier than the first definition. But
let's unveil what is hidden here. So Cantor means to[fn:2]

- establish some __membership__ criterion,
- gather things passing this __membership__,
- and call it a set.
 
Early tip: He's all about __membership__ and exactly how it works in
the world of math---although this doesn't necessary jump out at us at
this point. In math notation we can express this as
...
property or criteria, actual things passing this criteria can be
grouped together and called a set. As it turned out, Cantor was too
freewheeling with his grasp of sets. His definition allows us to
create sets out of the absolute void---just state a qualifying
property, no matter how crazy---and everything in the conceivable
universe with that property exists as a legitimate set. Trouble
started when a famous logician Bertrand Russell, using this loose
definition, came up with a devastating paradox. We'll go into
**[[https://en.wikipedia.org/wiki/Russell%27s_paradox][Russell's Paradox]]** later, but suffice it to say the math world was
shocked---and had to go back to the drawing board on many things long
taken for granted. Whole schools of logic came crashing down. But
we'll keep going with what is now called . And yet Cantor's theories,
no matter how prone to failure, are still what we study, the so-called
**[[https://en.wikipedia.org/wiki/Naive_set_theory][Naive set theory]]**, albeit with minor, behind-the-scenes tweaks. But
before we go on, let's get a better handle on just what's meant when
we say /property/, /predicate/, and /statement/.
...
of being a parent. In the form ~parent(X)~ with variable ~X~ it is now
considered a /predicate/. The predicate ~parent(X)~ is not yet true or
false until we provide a value for ~X~, then it becomes a
statement. Bringing this into our discussion of sets, the set of
~parent~'s can be expressed as[fn:3]

\begin{align*}
\exists S\, \forall X\, (X \in S \iff parent(X))
\end{align*}

In logic a predicate is described as a "verb phrase template" that
verbalizes a property. In the previous example ~parent~ becomes the
...
logic** (FOL), which we will dive into later. In effect, FOL is the
"operating system" or "grammar" on which theories like set theory
run. FOL provides the "grammar" for reasoning with these key
components:
...
- *Predicates (Verbs/Adjectives)*: Describe properties or
  relationships (e.g., $Man(x)$ or $Likes(x, y)$). These are the only
  components that carry factual meaning.
- *Connectives (Logical Glue)*: Symbols like $\land$ (and), $\lor$ (or), $\neg$
  (not), and $\implies$ (implies).
- *Quantifiers (Scope)*: $\forall$ (Universal - "for all") and $\exists$
  (Existential - "there exists").
- *Equality*: A special binary predicate built into FOL---which we
  discussed in a separate math holiday.

The great takeaway from this is FOL is the logic base---out of which
set theory is built. A theory has what is called an FOL /signature/
which is math-talk for a set of symbols. /The signature of set theory
consists of just one thing: membership $\in$/. This means every
statement in set theory is built by combining FOL symbols with the $\in$
symbol. This makes intuitive sense since set theory is all about what
belongs in a set. From now on be aware that when we have an expression
like
...
/this is actually an FOL expression/ because it is made up of the
building blocks of FOL listed above. Again, like we saw with Apostol,
...
We'll approach sets with a bit more exactness[fn:4]
...
⌜\\
𝖟𝕭: $\{1,2,3\}$ is the same as $\{ x \in \mathbb{N} \mid x < 4 \}$[fn:8]
...
duplicated item would just seem like a mistake.[fn:9] In set theory
there is the /[[https://en.wikipedia.org/wiki/Cardinality][cardinality]]/ of a set, i.e., what is the count of the
/unique/ elements of the set, /not/ counting duplicates.
...
/belong-belong/ or belong twice to the club.[fn:10] In the example
...
our daily workweek moods.[fn:11] \\
...
elements denoted by $x$ such that an $x$ can be an element from[fn:16]
$A$ *or* an element from $B$.[fn:17] But wait, *or* doesn't quite
...
together an infinite amount of **singleton**[fn:18] sets of the form
...
numbers $\mathbb{N}$ then what numbers of $\mathbb{N}$[fn:19] can we
...
operation of multiplication is not.[fn:20]
...
usually meant to be **ordered**.[fn:21] Again, idempotency of two
...
[fn:1] ...however often enough not as fully explored as with higher
math treatments.

[fn:2] We'll get to why he uses the term "comprehension" after some
preliminaries. Hint: it's a very non-intuitive, theoretical
mathematical use of the word.

[fn:3] In Prolog a predicate is lower-case and a variable is upper-case.

[fn:4] The following section comes from the very wonderful /[[https://slc.openlogicproject.org/slc-screen.pdf][Sets,
...
[fn:5] Recall, the math symbol $\in$ means /is in/ (membership) and $\notin$
means /is not in/.

[fn:6] With ~(1)~ we see the most basic mathematical notation for a
set, i.e., /all/ the sets' elements listed out in a row, separated by
commas, enclosed in a set of curly brackets.

[fn:7] Can we say this about words, e.g., are the words /hot/ and
/hoot/ the same? No. Words are not sets of letters, rather, /lists/ of
letters. More later.

[fn:8] Read, /$x$ such that $x$ is a natural number $\mathbb{N}$ and
$x$ is less than $4$/. This is known as the /roster/ method of set
notation.

[fn:9] No real "information" is contained in, say, listing milk
twice. Typically, no one would think "That means get /two/ milks."

[fn:10] ...paying double membership fees?

[fn:11]  Notice once again the nested parentheses. To be sure,
everything in Scheme is a list.

[fn:12] A Venn diagram of two sets /or/-ed, /union/-ed together. \\
...
[fn:13] $\cup$ is the symbol for union.

[fn:14] Could we have said, ...in $A$ *and* in $B$ *and* in both $A$
and $B$? Better not. More on why later.

[fn:15] (1) can be expanded for the union of more sets than just
...
[fn:18] As we saw above, a **singleton** (also known as a **unit set**
or **one-point set**) is a set with exactly one element.

[fn:19] $\mathbb{N}$ is the symbol for the /natural/ numbers, i.e., the
positive integers $1$, $2$, $3\ldots$ we use to count things.

[fn:20] In her 2023 layman's book /The Joy of Abstraction/
...
[fn:21] ...such as, e.g., a pair of /Cartesian coordinates/ $(a,b)$
...
[fn:22] We indicate the cardinality of a set $A$ by surrounding it
