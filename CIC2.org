...
** Why discrete math for high schoolers?

/Where/ is math? Not what is math, rather, where is math? Before
digital computers arrived in the mid-twentieth century, math could be
found in books, papers, on chalk boards. embodied in mechanical
...
called mathematicians, physicists, and engineers. Fast-forward to
today where tens of billions of computing devices---nearly all much
more powerful than the first NASA computers sending men into orbit and
to the moon---are doing computations big and small in every corner of
the world. Most of these calculations are relatively simple, but many
are quite complex. Today math overwhelmingly resides in computers, and
learning the skills necessary to do math and science with computers
will only increase in importance as we advance into the future.

Classic K-12 textbook math was designed to be tractable with pencil
and paper, later to include the pocket calculator. However, starting
in the 1950s universities and research organizations began migrating
math over to the digital, Boolean-algebra-based silicon brains, and so
began the new world of numerical-computational math. And yet K-12
education trudged along with texts, pencil, paper, finally adding
"graphing calculators." For math and STEM education it's almost as if
there are no such things as computers. Today we can no longer
...
into running code on very fast machines. These facts are what drives
this effort. DMfHS will attempt to create an on-ramp to the digital
age by creating a fundamental understanding of what is really going on
with computers.

** What is discrete math?

#+name: 0d9fda57-b635-43e7-a06c-90b198bb75fb
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
(define (decimal-to-binary n)
;; Converts a non-negative integer n to a binary list (MSB first).
;; Returns '(0) for n=0.
  (if (< n 0)
...
#+RESULTS: 0d9fda57-b635-43e7-a06c-90b198bb75fb
: #<unspecified>

#+name: fe2285a7-f790-43f8-a226-9224de1107ce
#+begin_src scheme :session *littleschemer* :results verbatim :exports both
