...
Starting typically with middle or high school algebra, math teachers
begin asking students to "show their work" when solving homework
problems. Teachers want to see all the /steps/ the student took to
arrive at the final answers. Why? By listing every step we demonstrate
more clearly our process of solving a problem. And if we didn't get
the right answer, we can better find where we took a wrong
...
hunches, and guesswork. Let's take "show your steps" further by using
the following foundational **rules** or **[[https://en.wikipedia.org/wiki/Axiom][axioms]]**[fn:1] of
algebra. Some of these may be familiar, others not, and some will
seem---odd---as in, Why do we need to worry about this stuff?
...
Great, but what's this for? So let's solve an algebra expression for
$x$, adding in justifications from the properties above for each
step[fn:2]
...
/justified/ each **rewrite**[fn:3] of each step by attaching one of
...
you to go into such depth. But as mentioned above, these axioms can be
the basis of all algebraic operations. Here we begin to take algebra
out of conditioned, rote response and **operationalize** it.

So far so strange... Typically, when you first learned algebra you did
not worry about such formal, systematic methods. Instead, you watched
and followed what the teacher was doing, picked up on the impromptu
rules, looked for patterns and clues in the examples, in effect, you
learned in an instinctive, pattern-recognition way to solve problems
after much imitation and repetition. You /internalized/ the methods
without going into any further depth. However, starting at the
university, you will be required to grasp things at a more systematic
level. You'll be introduced to the greater world of **[[https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)][mathematical
formalism]]**.

With axiomatic mathematics we start with a set of axioms, i.e.,
givens, rules, laws, statements to be taken as the most basic,
atomic[fn:4] facts, then employ **[[https://en.wikipedia.org/wiki/Deductive_reasoning][deductive reasoning]]** to /prove/
...
instead, the mantra is "We do enough problems until we catch on." Now,
however, the goal changes from "get problems solved" to attaining a
higher, more systematic level of understanding, one where we will
eventually be able to go out into the world and create new
...
So real math is bodies of theorems that have been /proven/ through
axioms. Here's Kenneth Rosen from his /[[https://www.mheducation.com/highered/product/Discrete-Mathematics-and-Its-Applications-Rosen.html][Discrete Mathematics and Its
Applications]]/ a widely-used college textbook in computer science
departments where he emphasizes **[[https://en.wikipedia.org/wiki/Mathematical_proof][proof]]** [fn:5]
...
Let's take a quick /math holiday/[fn:6] to give an example of how
...
failed to /prove/ it will always terminate and not just keep running.
...
Running this computation for $7$, we do get to $1$, indeed, in sixteen
steps. Then we're stuck in a so-called "trivial loop," i.e., at Step
16 we get $1$, but then by strictly following the recipe we keep
going: $1$ is odd, hence, multiply by $3$ and add $1$ getting $4$,
which is even, then divide by $2$, which is $2$, then divide by $2$
and we're back at $1$ --- over and over. If we've written a program
for this and our software is not told to break the loop, the reducing
will go on indefinitely. Knowing if a program ends properly or not is
a huge issue in the computer world.

Mathematicians have not /proved/ the Collatz conjecture always
terminates at $1$ for all $n$, therefore, we cannot know for certain
whether any given number can be reduced to $1$. This is a good example
of why proving conjectures, statements is crucial. Now, could a
computer simply keep trying ever bigger numbers? Yes, but that's
what's known as applying "brute force," i.e., not really
desirable. With the Collatz conjecture they have applied brute
force---out to $2.36 \times 10^{21}$, a colossal number. Again, this is a good
example of a central issue in math and computer science, namely, Is
there a resolution, a solution to a computation---at least in a
reasonable amount of time and computing power? In the real world of
math and computer science we seek provable outcomes. This means we
need to start with a proof. The question of what is computable and
what is not began in earnest, at least theoretically, on paper, back
in the 1930s /long before a single electronic computing device
actually existed/.

** Algebra from axioms

Tom Apostol in his /[[https://en.wikipedia.org/wiki/Calculus_(Apostol_books)][Calculus...]]/[fn:7] textbook starts with just this
sort of foundation-laying formalism for calculus[fn:8] when he lays
out the basics of what we've taken for granted for years, namely,
simple adding and multiplying
...
he calls **field axioms**. Hmm. Doesn't this whole business seem
tedious, over-the-top? Can't we just /do/ adding and subtracting and
not get so lost in the weeds? It's almost like he's explaining adding
and subtracting to a robot or a computer. Hold that thought. But then
Apostol boldly states,
...
We said this before, and yes, we're attempting to /deduce/ algebra
from axioms. But again, why all this formalistic stuff? One way to
answer this is to try to think about computers and how math might be
done on them. Computers run **[[https://en.wikipedia.org/wiki/Algorithm][algorithms]]** expressed in computer code
written in computer programming languages. We'll explore algorithms as
we go, but suffice it to say, when a computer "figures stuff out" it
is using carefully constructed, mathematically-based
algorithms. Here's what [[https://gemini.google.com/app][Google Gemini AI]] said
...
above of sum and product in the real numbers into lines of computer
code, into algorithms that can do algebra for us? Something like
creating a cookbook of recipes, no? Again, hold that thought, because
it is central to everything we're doing here. /The connection between
...
algebra more like a circus animal learns trick, but a computer
cannot.[fn:9]
...
establishing basic, given facts, axioms, then using them to build
theorems, the body of working parts.

In the algebra example above where we simplified $5x - 12 = 3(x + 2)$,
one step was justified, matched with one of the /Properties of
Equality/ (PoE) arguments. But then some of these PoEs Apostol says
are technically theorems and, as such, must first be proven. His first
**Theorem 1.1**, the /Cancellation law for addition/, (compare with
/Addition property of equality/ above) states[fn:10]

⇲@@html:<font color = "#0d3db3">@@ If $a + b = a + c$, then $b = c$. In
particular, this shows that the number $0$ of Axiom 4 is unique.
@@html:</font>@@

Perhaps you intuitively get what is meant, but you may still wonder
how this definition, as it was just given, creates, enables the
cancelling like things on each side of an equation. Also, he ties in
his Axiom 4 saying the uniqueness of $0$ an issue. Seemingly, Apostol
isn't that clear. Then comes his proof:[fn:11]

⇲@@html:<font color = "#0d3db3">@@ Let us assume $a + b = a + c$ is
true. By Axiom 5 (see /Existence of negatives/ above), there is a
number such that $y + a = 0$. Since sums are uniquely
determined,[fn:12] we have $y + (a + b) = y + (a + c)$. Using the
associative law, we obtain $(y + a) + b = (y + a) + c$ or $0 + b = 0 +
c$. But by Axiom 4 (see /Existence of identity elements/ above) we
have $0 + b = b$ and $0 + c = c$, so that $b = c\:$. Notice that this
theorem shows that there is only one number having the property of $0$
in Axiom 4. In fact, if $0$ and $0^\prime$ both have this property, then
$0 + 0^\prime = 0$ and $0 + 0 = 0$. Hence $0 + 0^\prime = 0 + 0$ and, by the
cancellation law, $0 = 0^\prime$.

@@html:</font>@@

Supposedly, the idea of "whatever you do to one side, you have to do
the same to the other side" or "balancing the equation" has just been
formally explained and proved---even if it's hard to grasp how. Above
it was a given, an axiom; but Apostol calls it a theorem and, thus,
must be proved. Again, when we hear "cancel on both sides" we think
"just cross things out." But no, that won't do any longer, rather, we
have to prove we are allowed to do such a rewrite and base our proof
on axioms. Again, we are /operationalizing/. Why? Because computers
can't (yet) learn algebra like we do; for the computer algebra must be
adapted for the realm of computer operations.

Let's look at this issue from another angle by asking an AI (in this
case [[https://grok.com/][Grok]]) to explain balancing an equation in an axiomatic-based
way. Some of this repeats the above discussion; other things will be
different, i.e., a new perspective, a deeper dive. Peruse the
following sections and their discussions[fn:13]
...
can be grounded in these so-called /Axioms of Equality/, which we saw
above. However, the /Substitution Principle/ is new and is supposedly
key. Whether obvious right now or not, it is what allows us to replace
equals with equals in expressions, which goes to the heart of
algebra. How then?
...
/Substitution Principle/.[fn:14] So we'll start again with Apostol's
Theorem 1.1: @@html:<font color = "#0d3db3">@@If $a + b = a + c$, then
$b = c$.@@html:</font>@@

1. Given: $a + b = a + c$
2. The substitution step: Because the expression $(a + b)$ is
   identical in value to $(a + c)$, we can "do the same thing" to
   both. /And so we choose to **add** $(-a)$ to the left of both
   sides/.
...
opposite $(-a)$. And this axiom-based approach lends itself to
computers and their algorithmic ways---although as we stated above,
the whole idea of term rewriting is a big and complex thing.
...
The three axioms before the Substitution Principle are making exacting
points about just what equality is. In higher math the concept of
equality, of /sameness/ goes much deeper and is more precise and
nuanced than our everyday intuitive understanding of /equals/---as we
saw with Leibniz's very theoretical [[https://en.wikipedia.org/wiki/Identity_of_indiscernibles][Indiscernibility of Identicals]]. In
one formal definition, equality is called an /equivalence relation/ on
numbers. We'll look into exactly what that could mean, as well as what
a /relation/ is in the higher math sense soon. Let's take a quick
holiday to see



Grok's answer is
compelling us to tackle some higher math, which is based on **[[https://en.wikipedia.org/wiki/Set_theory][set
theory]]** and **[[https://en.wikipedia.org/wiki/Mathematical_logic][mathematical logic]]** developed in the latter half of
the nineteenth century on into the twentieth to be an exhaustively
thorough and foundational basis for math, upon which all math could
hopefully be axiomatically built. We will weave both sets and logic
into our presentations as we go. One part of math logic is
**[[https://en.wikipedia.org/wiki/First-order_logic][first-order logic]]** or FOL. Again, nothing in depth now, but we'll
need a quick intro to FOL to understand what's being said about
/predicates/ in the /Substitution Principle/ above.

Grok's first axiom is **reflexivity**, which might seem
/vacuous/[fn:15] and unnecessary to the layperson. Of course any given
thing is the same, /equal/ to itself. But consider this wording of
reflexivity, i.e., /a relation each thing bears to itself and nothing
else/. We are stating this explicitly to be thorough. Believe it or
not, having reflexivity as an axiom is necessary in certain
situations.
...
and they're equality is not changed. Again, you were just conditioned
to accept this, but here it is formally stated.

And now let's take a math holiday to look into FOL so we can
understand what's going on with predicates and why $\forall x \, (x = x)$ is
just another version of $a = b$.
...
Actually, when Logic is studied we usually begin with **[[https://en.wikipedia.org/wiki/Propositional_logic][Propositional
Logic]]**, then FOL. But here we'll just touch on FOL
...
axioms[fn:16] ... and EE has been, in one form or another, a part of
...
situation[fn:17] and understand why something about it is /necessarily/
...
givens.[fn:18] For example, the last notion, N5, seems to say something
...
useful math is solidly, logically built. Higher math[fn:19] is the
...
i.e., relying on these axioms[fn:20]
...
a set of tools.[fn:22]

** Axiomatic Scheme

And now with no further ado we will list out TLS's version of
"axiomatic givens" starting with /The Ten Commandments/ below[fn:23]
...
exactly how to write a Scheme program.[fn:24] But then if you've read
...
omnibus[fn:25] /list processing/ machinery. In fact Scheme's
...
higher math, as well as CS's Discrete Math.[fn:26] And so many books
...
elements of the set, /not/ counting duplicates.[fn:27]
...
/belong-belong/ or belong twice to the club.[fn:29] In the example
...
our daily workweek moods.[fn:30] \\
...
elements denoted by $x$ such that an $x$ can be an element from[fn:35]
$A$ *or* an element from $B$.[fn:36] But wait, *or* doesn't quite
...
together an infinite amount of **singleton**[fn:37] sets of the form
...
numbers $\mathbb{N}$ then what numbers of $\mathbb{N}$[fn:38] can we
...
operation of multiplication is not.[fn:39]
...
usually meant to be **ordered**.[fn:40] Again, idempotency of two
...
[fn:1] **axiom**: From the Greek /axíōma/ meaning /for that which
commends itself as evident/.

[fn:2]  𝖟𝕭 is the German abbreviation for /zum Beispiel/ meaning "for
example," a nod to all the German-speaking contributors to higher
math.

[fn:3] An entire field of higher math is devoted to just this sort of
...
[fn:4] **atomic**: In a philosophical, mathematical context, the term
...
[fn:5] Lots more proving of things as we go.

[fn:6] A /math holiday/ is a euphemism we use meaning to take a
detour, go down a rabbit hole in order to delve into some math topic
necessary to advance the discussion.

[fn:7] Apostol's college-level /Calculus, Volume 1: One-variable
calculus, with an introduction to linear algebra/ straddles the line
between a Calculus and a deeper, more theoretical Analysis book.

[fn:8] Both algebra and calculus are based in the realm of real
numbers, which uses the symbol $\mathbb{R}$, hence, Apostol's
clarification. He also refers to the /set/ of real numbers. That's
from **set theory**, soon to be explored...

[fn:9] ...or can it? AI based on neural networks and large language
...
logic back into the picture.

[fn:10] One notorious fact about math is how different authors use
...
[fn:11] Again, this is deep end. Be aware that Apostol's /Calculus.../
...
[fn:12] ... /sums are uniquely determined/? What does that mean? Refer
back to his intro about sums and products. Don't panic. More later.

[fn:13] Content starting with a ⇲ symbol and in blue is what Grok has
told us (with some editing, addendum for clarity). Following in black
are more discussion and explanations.

[fn:14] [[https://en.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz][Leibniz]]'s Law (also called **Indiscernibility of Identicals**)
...
[fn:15] In math and logic a **vacuous truth** is a statement that is
...
[fn:16] A *[[https://en.wikipedia.org/wiki/Conjecture][conjecture]]* is a tentative *[[https://en.wikipedia.org/wiki/Proposition][proposition]]* that may evolve to
...
[fn:17] For example the (in)famous /[[https://en.wikipedia.org/wiki/Pons_asinorum][pons asinorum]]/ in /Euclid's
Elements/ \\
...
[fn:18] In mathematical logic the idea of /implication/ is very
...
[fn:19] /Higher math/ in the U.S. typically means those college math
...
[fn:20] These axioms relate to Linear Algebra.

[fn:21] ... /quod erat demonstrandum/, meaning "that which was to be
demonstrated".

[fn:22] Go ahead and take a "math holiday" by touring [[http://aleph0.clarku.edu/~djoyce/elements/bookI/cn.html][this page]] by
...
[fn:23] Do /not/ worry about what all this means. In time we will
...
[fn:24] ...if we could, would each program we write technically be a
...
[fn:25] ...containing or including many items.

[fn:26] ...although it may be veiled or only partially explored.

[fn:27] We indicate the cardinality of a set $A$ by surrounding it
with vertical pipes: $|\,A\,|$ This is similar to the idea of a
number's /absolute value/ which uses the same vertical bar symbols.

[fn:28] 𝖟𝕭 is German for /zum Beispiel/ or /for example/.

[fn:29] ...paying double membership fees?

[fn:30]  Notice once again the nested parentheses. To be sure,
everything in Scheme is a list.

[fn:31] A Venn diagram of two sets /or/-ed, /union/-ed together. \\
...
[fn:32] $\cup$ is the symbol for union.

[fn:33] Could we have said, ...in $A$ *and* in $B$ *and* in both $A$
and $B$? Better not. More on why later.

[fn:34] (1) can be expanded for the union of more sets than just
two, e.g., $A \cup B \cup C \cup \ldots$.

[fn:35] The symbol $\in$ means "in" or "member of."

[fn:36] $\lor$ is the symbol for **[[https://en.wikipedia.org/wiki/Logical_disjunction][logical or]]**, aka **logical
disjunction**. Take a crack at the Wikipedia article.

[fn:37] As we saw above, a **singleton** (also known as a **unit set**
or **one-point set**) is a set with exactly one element.

[fn:38] $\mathbb{N}$ is the symbol for the /natural/ numbers, i.e., the
positive integers $1$, $2$, $3\ldots$ we use to count things.

[fn:39] In her 2023 layman's book /The Joy of Abstraction/
...
[fn:40] ...such as, e.g., a pair of /Cartesian coordinates/ $(a,b)$
...
[fn:43] Despite containing actual Scheme programs, i.e., real code, TLS
...
[fn:42] Go [[https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)][here]] to see the Wikipedia discussion about mathematic
formalism---although it's a bit formal...

[fn:41] Βιγ Σξηεμερ: Big Schemer; Λιττλε Σξηεμερ: Little Schemer in
Greek letters.
