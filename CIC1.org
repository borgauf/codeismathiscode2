...
One example of not really understanding, rather, "getting used to"
math might be this mathematical logic description of
**[[https://en.wikipedia.org/wiki/Atomic_formula][atomic expressions]]** in the programming language **[[https://en.wikipedia.org/wiki/Standard_ML][Standard
ML]]**
...
What does all this mean? Well, the first line, $C \vdash \text{atexp}
\Rightarrow \tau$, means

@@html:<font color = "#0d3db3">@@
⥤ Under the Context $C$, the /atomic expression/ $atexp$ is inferred
to have type $\tau$.@@html:</font>@@

You probably learned this in Kindergarten or first grade and just
forgot it, no?

These logic diagrams come from ML's /formal specification/ presented
in /The Definition of Standard ML/ and are a portion of the actual
mathematical logic behind the programming language Standard ML. You
are looking at /[[https://en.wikipedia.org/wiki/Sequent_calculus][sequent calculus]]/ (or /natural deduction/) notation,
which is the standard language for formal semantics applications like
programming languages. Think "schematic diagrams" for the internal
workings of a programming language. No worries, we won't be tackling
this sort of thing until much later. But as we move along, the point
will be relentlessly driven home that /modern computer technology is
tightly intertwined with higher math and mathematical logic/. At this
stage in the game, just gaze in awe. This is not meant to scare,
rather, to intrigue you. Again, be amazed, be very amazed. But be
aware that this very thing is indeed studied in college computer
science departments. And preparing you for college-level comp-sci is
what this is all about. But you might counter, I don't necessarily
want to study comp-sci. To that we say, Yes you do---at least the
basics. /Everybody should know the basics of the most important
technology the human race has ever created/. Let's get started.
...
it into more systematic, rule-based procedures. We need to know
__why__, not just guess-and-test our way along. Again, why?  /Because
computability in general and computer operation specifically must be
grounded in logical entailment/. Higher math is all about packaging
and handling logic and its consequences. To be sure, experiencing how
higher math "hangs together" can be a marvel, discovering all the
interlocking, interconnected logical consequences, though initially it
can be a frustrating descent into confusion---until your next eureka
moment lands. What follows in this, our intro chapter, will seem
challenging; but in the spirit of math text first chapters, /do not
panic/, do not take this too seriously, do not attempt to rote learn
or memorize; rather, just go along for the ride: read, re-read, even
do your own investigations.[fn:2] Above all, don't worry, everything
mentioned here will be fleshed out, relevance thoroughly established
...
generalization offer us the deepest possible understanding of a
subject. Recall with Einstein's [[https://en.wikipedia.org/wiki/General_relativity][General relativity]] that gravity, an
everyday, real-world phenomenon, has, through Einstein, become an
/artefact/, literally a logical consequence of a mathematical
system. Bottom line: /Einstein gave us a much deeper and expansive
understanding of what physical reality is./ That's an incredible boon,
nothing short of a superpower we humans have gained over life here in
the universe.[fn:3]
...
[fn:5]  𝖟𝕭 is the German abbreviation for /zum Beispiel/ meaning "for
...
[fn:27] The symbol $\iff$ is shorthand for /if and only if/. It is
...
[fn:35] Here the symbol ⊕ means any binary operation, e.g., addition,
subtraction...
