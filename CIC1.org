...
chapter where we'll explore just how different higher math can be from
the basic math of K-12. While basic math---all the way through
Calculus and Differential Equations in college---is about getting
solutions to problems, higher math can be downright
/philosophical/. But why do we have to worry about higher math in the
first place? As we've discussed in the Preface, the theory that grew
alongside the development of digital computers has always been
rigorous and higher-math-based. /It had to be./ And yes, our
catchphrase, /formalize to operationalize/ is central to
everything. In addition, we need to move away from intuition,
conditioned instincts, hunches, and guesswork.
...
it into more systematic, rule-based procedures. We need to go beyond
conditioned instincts and hunches. We need to know why, not guess
how. Again, why?  /Because computability in general and computer
...
consequences. To be sure, how higher math "hangs together" can be a
marvel, though initially a frustrating descent into confusion---until
your next eureka moment lands. What follows in this, our intro
chapter, will seem challenging; but as this is in the spirit of math
text first chapters, /do not panic/, do not take this too seriously,
do not attempt to rote learn or memorize; rather, just go along for
the ride: read, re-read, even do your own investigations.[fn:1] Above all,
don't worry, everything mentioned here will be fleshed out, relevance
thoroughly established in subsequent lessons.

Higher math is all about creating elegant, self-contained,
self-referential, soup-to-nuts bodies of theories that /scale/. Reach
a higher plateau, see much farther. Higher math always seeks to
/generalize/. Why? Because consistent systems of symbolism and
generalization offer us deep understanding of a subject. Recall with
Einstein's General Relativity that gravity, an everyday, real-world
phenomenon, has, through Einstein, become an /artefact/, literally a
logical consequence of a mathematical system. /Einstein gave us a much
deeper understanding of what physical reality is./ That's an
incredible boon, nothing short of a power we humans have gained over
life here in the universe.[fn:2]

Let us begin with that odd-seeming request many math teachers ask of
their algebra students when solving homework problems: "Show your
work." Mysteriously, teachers suddenly want to see all the /steps/
their students took to arrive at final answers. Why? It could be that
by listing every step we demonstrate more clearly our process of
solving a problem. And if we didn't get the right answer, we can
better narrow down where we took a wrong turn ... or so you're
told. Let's take "show your steps" further by involving the
foundational **rules** or **[[https://en.wikipedia.org/wiki/Axiom][axioms]]**[fn:3] of algebra as seen from the
higher-math perspective.  Throughout /Computing in Context/ we
emphasize /formalize to operationalize/, and this is where we'll
start. Below are /properties/ we can consider axioms or given
facts. These can be found in this basic form in many textbooks. You've
probably seen most of them---and not really known what they were for
... as in, Why do I need to worry about this stuff? Peruse, then we'll
look at how to use them, as well as other versions.
...
/justified/ each **rewrite**[fn:5] of each step by attaching one of
...
algebraic operations/. Now we begin to /operationalize/ algebra.
...
atomic[fn:6] facts, then employ **[[https://en.wikipedia.org/wiki/Deductive_reasoning][deductive reasoning]]** to /prove/
...
computer science departments where he emphasizes **[[https://en.wikipedia.org/wiki/Mathematical_proof][proof]]** [fn:7]
...
Let's take a quick in situ /math holiday/[fn:8] to give an example of
...
mathematically but not actually compute them. A lot more later.[fn:9]
...
tools to analyze computations, even prove[fn:10] what a computation
...
starts with higher math-style foundation-laying for Calculus,[fn:12]
...
...then he gives what he calls the **field axioms**[fn:14]
...
of /symbolic manipulation/.[fn:15] Consider this quote from the
...
can be generalized by letter symbols as[fn:16]
...
cancellation.[fn:19] Does his Theorem 1.1 rely on /Axiom 5, Existence
...
zero after all, which proves[fn:21] it is impossible to have more than
...
Let's see another version of Apostol's Theorem 1.1:[fn:22]

\begin{align*}
a + c = b + c \implies a = b
\end{align*}

Now, here is a step-by-step summary[fn:23]
...
$(a + c) + (-c) = (b + c) + (-c)$/. We've added $-c$ to both sides to
cancel $c$. But we need a really solid law to allow this. Subtle but
...
followed up by many theorems we can assume deduced/derived from these
/field/ axioms.[fn:24] And there you have Calculus! Or at least the
solid foundations upon which you may start to build Calculus. Note how
Lee starts off with equality---which are not exactly the same as the
generic Properties of Equality given at the beginning of this
chapter. Following is with Lee's same items, yet more generally,
higher-math stated[fn:25]
...
Let's compare Lee's /Substitution/ with our more abstracted version
above
...
This is from an college intro Calculus course that isn't trying to be
higher math. For example, what allows us to say, /if by reflexivity $a
= a$, then $a + c = a + c\;$/? Yes, reflexivity is definitely involved
and central, but that's not all of it. His version of the
/Cancellation Law/ (Theorem 1) would seem to back into this. The
/Addition Property/ from the generic /Properties of Equality/ version
at the beginning of the chapter covers this as well---but with four
more than our latest PoE list. Are, therefore, /Addition, Subtraction,
Multiplication, Division, Square root/, and /Zero product/ properties
now being ignored, or, are they actually contained, made redundant by,
collapsed down into the /Substitution Principle/? Let's keep going.

Your algebra training might be telling you to simply

- let $x = (a + c)$
- by Reflexivity $x = x$
- $\therefore (a + c) = (a + c)$

Is this really all the /Substitution Principle/ is, i.e.,
/substituting/ symbols? In the fancier version of the /Substitution
Principle/ above we're confronted with the higher-math concepts of
/property/ and /predicate/. But with Lee's /Substitution/, again, he
only says
...
reconcile Lee's and our more general wordings of /Substitution/
involving predicates? They look and sound completely different. Let's
begin by analyzing what is meant by a "mathematical statement." As
we'll explore later when we go into math logic, a so-called
/mathematical statement about a variable $x$/ means essentially the
same as a /predicate $P(x)$/. Lee does not really explain what is
meant by a statement, but as we'll soon learn /a statement is anything
that can be evaluated to **true** or **false**,/ and this is exactly
what a proposition or predicate is.  Furthermore, Lee's wording
emphasises /interchangeability/, i.e., if we know $a = b$, the /truth/
of $a$ in any operation is not dependent on the symbol, the label
$a$. Hence, the notation $P(a) \iff P(b)$[fn:26] is the high-math,
symbolic way of stating what Lee does

- P(x), read /a property $P$ of $x$/ represents Lee's /any
  mathematical statement/, and
- $P(a) \iff P(b)$, or, /the property of $a$ is true if and only if
  that same property of $b$ is true/ translates as Lee's /without
  affecting that statement’s truth value/. Or /If it was true for $a$
  it must be true for $b$, __and vice versa__.

Here's a quick example using Scheme code to demonstrate what a
/predicate/ does in the wild
...
principle developed originally by Gottfried Wilhelm Leibniz.[fn:27]
...
mathematics.[fn:28],[fn:29] Now, does this have anything to do with
...
very thorough mathematical logic way.[fn:30]
...
different objects.[fn:31] $\forall x \, (x = x)$ keeps sanity by insisting
...
/Substitution Principle/.[fn:32] So we'll start again with Apostol's
...
called an /equivalence relation/ on numbers.[fn:33]
...
/vacuous/[fn:34] and unnecessary to the layperson. Of course any given
...
and existence/ about $=$ in logic beingis just another version of $a = b$.[fn:35]
...
axioms[fn:36] ... and EE has been, in one form or another, a part of
...
situation[fn:37] and understand why something about it is /necessarily/
...
givens.[fn:38] For example, the last notion, N5, seems to say something
...
useful math is solidly, logically built. Higher math[fn:39] is the
...
i.e., relying on these axioms[fn:40]
...
a set of tools.[fn:42]

** Axiomatic Scheme

And now with no further ado we will list out TLS's version of
"axiomatic givens" starting with /The Ten Commandments/ below[fn:43]
...
exactly how to write a Scheme program.[fn:44] But then if you've read
...
omnibus[fn:45] /list processing/ machinery. In fact Scheme's
...
higher math, as well as CS's Discrete Math.[fn:46] And so many books
...
elements of the set, /not/ counting duplicates.[fn:47]
...
/belong-belong/ or belong twice to the club.[fn:49] In the example
...
our daily workweek moods.[fn:50] \\
...
elements denoted by $x$ such that an $x$ can be an element from[fn:55]
$A$ *or* an element from $B$.[fn:56] But wait, *or* doesn't quite
...
together an infinite amount of **singleton**[fn:57] sets of the form
...
numbers $\mathbb{N}$ then what numbers of $\mathbb{N}$[fn:58] can we
...
operation of multiplication is not.[fn:59]
...
usually meant to be **ordered**.[fn:60] Again, idempotency of two
...
[fn:1] ...e.g., try to understand as much as you can of the Wikipedia
articles linked.

[fn:2] ...and this power is often jaw-dropping. The ways human
...
[fn:3] **axiom**: From the Greek /axíōma/ meaning /for that which
commends itself as evident/.

[fn:4]  𝖟𝕭 is the German abbreviation for /zum Beispiel/ meaning "for
...
[fn:6] **atomic**: In a philosophical, mathematical context, the term
...
[fn:7] Lots more proving of things as we go.

[fn:8] We use /math holiday/ as a euphemism meaning to take a detour,
...
[fn:9] This goes to the famous **[[https://en.wikipedia.org/wiki/Entscheidungsproblem][Entscheidungsproblem]]**: In
...
[fn:10] [[https://en.wikipedia.org/wiki/Mathematical_proof][Mathematical proof]]: Knowing whether something is true or false
...
[fn:11] In her /Recursive Functions in Computer Theory/, Rozsa Peter
...
[fn:13] Notice below how he only mentions /addition/ and
/multiplication/. Why? Could it be that /subtraction/ and /division/
can be /derived/ from adding and multiplying?

[fn:14] A **[[https://en.wikipedia.org/wiki/Field_(mathematics)][field]]** is like a higher math bag containing, A), a set of
...
[fn:15] Some treatments substitute the symbols $\oplus$ for "addition" and
...
[fn:16] ...even though the second set of premises and conclusion below
suddenly seems automatic for any ~M~, ~S~, and ~P~ we may plug in,
absurdities can obviously arise.

[fn:17] Ada Lovelace (1815 - 1852) is considered to be the first
...
[fn:18] One notorious fact about math is how different authors use
...
[fn:19] Apostol is treating this as a theorem rather than an axiom
...
[fn:20] In higher math the symbol \therefore means /therefore/.

[fn:21] See [[https://en.wikipedia.org/wiki/Reductio_ad_absurdum][Reductio ad absurdum]]., or proving something true by
assuming it false, then showing it cannot be false after all.

[fn:22] Instead of Apostol's $a$ on both sides, we have $c$. The
symbol $\implies$ means /implies/.

[fn:23] Note with Step 2 $\exists$ means /there exists/ and $\mid$ means
/such that/: "There exists some $-c$ such that $c + (-c)$ equals $0$."

[fn:24] Lee lists out 34 theorems. Perhaps ask an AI chatbot to prove
one of Lee's theorems.

[fn:25] Remember what was said about /relations/ in our last ex situ
Math Holiday. For now that's good enough.

[fn:26] The symbol $\iff$ is shorthand for /if and only if/.

[fn:27] We introduce the higher-math notation $\forall x \; (x=x)$ where the
$\forall$ symbol means /for all/. Hence, /for all/ $x$, $x = x$.

[fn:28] The symbol ⊕ means any operation, e.g., addition, subtraction...

[fn:29] Besides the obvious ones, subtraction and division, with
matrix math commutativity is not always true.

[fn:30] Basically, Boole invokes /idempotency/. Imagine two buttons,
...
[fn:31] With computers the **state** of the internal memory caused by
...
[fn:32] [[https://en.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz][Leibniz]]'s Law (also called **[[https://en.wikipedia.org/wiki/Identity_of_indiscernibles][Indiscernibility of Identicals]]**)
...
[fn:33] We took a quick look at what a /relation/ is in our /Existence
and uniqueness/ math holiday.

[fn:34] In math and logic a **vacuous truth** is a statement that is
...
[fn:35] Actually, when math logic is studied we usually begin with
**[[https://en.wikipedia.org/wiki/Propositional_logic][Propositional Logic]]**, then FOL. But here we'll just quickly touch
on FOL.

[fn:36] A *[[https://en.wikipedia.org/wiki/Conjecture][conjecture]]* is a tentative *[[https://en.wikipedia.org/wiki/Proposition][proposition]]* that may evolve to
...
[fn:37] For example the (in)famous /[[https://en.wikipedia.org/wiki/Pons_asinorum][pons asinorum]]/ in /Euclid's
Elements/ \\
...
[fn:38] In mathematical logic the idea of /implication/ is very
...
[fn:39] /Higher math/ in the U.S. typically means those college math
...
[fn:40] These axioms relate to Linear Algebra.

[fn:41] ... /quod erat demonstrandum/, meaning "that which was to be
demonstrated".

[fn:42] Go ahead and take a "math holiday" by touring [[http://aleph0.clarku.edu/~djoyce/elements/bookI/cn.html][this page]] by
...
[fn:43] Do /not/ worry about what all this means. In time we will
...
[fn:44] ...if we could, would each program we write technically be a
...
[fn:45] ...containing or including many items.

[fn:46] ...although it may be veiled or only partially explored.

[fn:47] We indicate the cardinality of a set $A$ by surrounding it
with vertical pipes: $|\,A\,|$ This is similar to the idea of a
number's /absolute value/ which uses the same vertical bar symbols.

[fn:48] 𝖟𝕭 is German for /zum Beispiel/ or /for example/.

[fn:49] ...paying double membership fees?

[fn:50]  Notice once again the nested parentheses. To be sure,
everything in Scheme is a list.

[fn:51] A Venn diagram of two sets /or/-ed, /union/-ed together. \\
...
[fn:52] $\cup$ is the symbol for union.

[fn:53] Could we have said, ...in $A$ *and* in $B$ *and* in both $A$
and $B$? Better not. More on why later.

[fn:54] (1) can be expanded for the union of more sets than just
two, e.g., $A \cup B \cup C \cup \ldots$.

[fn:55] The symbol $\in$ means "in" or "member of."

[fn:56] $\lor$ is the symbol for **[[https://en.wikipedia.org/wiki/Logical_disjunction][logical or]]**, aka **logical
disjunction**. Take a crack at the Wikipedia article.

[fn:57] As we saw above, a **singleton** (also known as a **unit set**
or **one-point set**) is a set with exactly one element.

[fn:58] $\mathbb{N}$ is the symbol for the /natural/ numbers, i.e., the
positive integers $1$, $2$, $3\ldots$ we use to count things.

[fn:59] In her 2023 layman's book /The Joy of Abstraction/
...
[fn:60] ...such as, e.g., a pair of /Cartesian coordinates/ $(a,b)$
...
[fn:63] Βιγ Σξηεμερ: Big Schemer; Λιττλε Σξηεμερ: Little Schemer in
Greek letters.

[fn:62] Go [[https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)][here]] to see the Wikipedia discussion about mathematic
formalism---although it's a bit formal...

[fn:61] Despite containing actual Scheme programs, i.e., real code, TLS
