...
ask students to "show their work" when solving homework
problems. Teachers want to see all the /steps/ the student took to
arrive at the final answer. Why? Because by listing every step in
solving a problem we demonstrate our clear understanding of the whole
process. And if we didn't get the right answer, we can better find
where we took a wrong turn. Showing all steps is about moving towards
a solution in a verifiable way and leaving behind intuition,
conditioned instincts, hunches, and guesswork. Let's take "show your
steps" further with these foundational **rules** or **[[https://en.wikipedia.org/wiki/Axiom][axioms]]**[fn:1]
of algebra. Some of these may be familiar, others not, and some will
seem---odd---as in, Why do we need to worry about that?
...
--- /Distributive Law, Addition property of equality/, etc.[fn:2] Now,
it's highly doubtful any middle or high school algebra teacher
discussed algebra in terms of axioms or asked you to go into such
depth. But as mentioned above, upon these axioms can be based the
complete operations of doing algebra. In other words, we can begin to
...
looked for clues in the examples, you learned /The Way/ over and
over,and, basically, /internalized/ the methods without going any
further. Again, you were /conditioned/ to do algebra. And this is how
the vast majority of primary and secondary education mathematics is
taught, i.e., students are given next to none of the underlying,
theoretical math. There is no emphasis on **[[https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)][mathematical
formalism]]**. Why? The goal was not to understand math at any
fundamental or philosophical level, rather, to "solve problems."  But
as we're saying, this approach has an expiration date in your STEM
career.

With **axiomatic mathematics** we start with a set of axioms,
"givens," rules, laws, statements to be taken as the most basic facts,
then employ **[[https://en.wikipedia.org/wiki/Deductive_reasoning][deductive reasoning]]** to /prove/ further statements,
known as **[[https://en.wikipedia.org/wiki/Theorem][theorems]]**, which, taken together, become the body of that
particular branch of math. The axiom-to-theorems approach is
diametrically opposed to typical K-12 "muscle-memory" math where the
mantra is "Do enough problems and you'll catch on." The goal changes
from "get problems solved" to attaining a higher level of
understanding, one where we will eventually be able to create new
mathematics. Whether or not you see axiomatic systems directly
discussed---as is in higher math and computer science---real-world
STEM relies on this formalistic approach even when it's not
obvious. We will explore how math is actually built from theorems
derived deductively based on axioms. And hopefully you will begin to
...
Let's take a quick /math holiday/[fn:4] to give an example of why proof is
...
the $3n + 1$ problem, it is celebrated for its deceptive simplicity: a
child can understand the rules, yet the world’s greatest
mathematicians have failed to /prove/ it will always terminate for
nearly a century.

Starting with any positive integer $n$, follow these two steps
repeatedly---which, if done long enough, should eventually wind up at
the number $1$:
...
Running this reduction computation for $7$, we actually get stuck in
the "trivial loop," i.e., at Step 16 we get $1$ but then strictly
following the recipe we keep going: $1$ is odd, so multiply by $3$ and
add $1$ getting $4$, which is even, then divide by $2$ and we're back
at $1$. If our software is not told to break the loop, it will go on
indefinitely, no?  @@html:</font>@@

If mathematicians have not /proved/ the Collatz conjecture always
terminates at $1$ for all $n$, then we cannot know whether we can
always get to $1$. Hence, proving statements are crucial. Now, can the
computer simply keep trying ever bigger numbers? With the Collatz
conjecture they have---out to $2.36 \times 10^21$, a colossal number. This
is a good example of a central issue in math and computer science---is
there a resolution, a solution to a problem? In the real world of
math we seek proof based on axiomatic math. The limits of
computability is a big subject in world of modern computing.

Tom Apostol in his /[[https://en.wikipedia.org/wiki/Calculus_(Apostol_books)][Calculus...]]/[fn:5] textbook starts out with just
...
/operationalize/ something so the computer can do it?
...
one step was justified with /Properties of Equality/ arguments, some
of which for Apostol are technically theorems and as such, must first
be proven. His first **Theorem 1.1**, the /Cancellation law for
addition/, states
...
Then comes Apostol's proof:[fn:6]
...
equation" has just been formally explained and proved---even if it's
hard to grasp the full meaning just yet. Let's look at this issue from
another angle by asking an AI (in this case [[https://grok.com/][Grok]]) to explain balancing
an equation in an axiomatic-based way. Some of this repeats the above
discussion; other things will be different, i.e., a new perspective, a
deeper dive. Peruse the following sections and their discussions[fn:7]
...
understanding of /equals/. Formally, equality is an /equivalence
relation/ on numbers. We'll look into exactly what a /relation/ is in
the higher math sense soon. Grok's answer is compelling us to tackle
some higher math, which is based on **[[https://en.wikipedia.org/wiki/Set_theory][set theory]]** and **[[https://en.wikipedia.org/wiki/Mathematical_logic][mathematical
logic]]** developed in the latter half of the nineteenth century on into
the twentieth to be an exhaustively thorough and foundational basis
for math, upon which all math could be axiomatically built. We will
weave both sets and logic into our presentations as we go. One part of
math logic is **[[https://en.wikipedia.org/wiki/First-order_logic][first-order logic]]** or FOL. Again, nothing in depth
now, but we'll need an intro to FOL to understand what's being said in
the /Substitution Principle/ above.
...
/vacuous/[fn:8] and unnecessary to the layperson. Of course any given
...
axioms[fn:9] ... and EE has been, in one form or another, a part of
...
situation[fn:10] and understand why something about it is /necessarily/
...
givens.[fn:11] For example, the last notion, N5, seems to say something
...
useful math is solidly, logically built. Higher math[fn:12] is the
...
i.e., relying on these axioms[fn:13]
...
a set of tools.[fn:15]

** Axiomatic Scheme

And now with no further ado we will list out TLS's version of
"axiomatic givens" starting with /The Ten Commandments/ below[fn:16]
...
exactly how to write a Scheme program.[fn:17] But then if you've read
...
omnibus[fn:18] /list processing/ machinery. In fact Scheme's
...
higher math, as well as CS's Discrete Math.[fn:19] And so many books
...
elements of the set, /not/ counting duplicates.[fn:20]
...
/belong-belong/ or belong twice to the club.[fn:22] In the example
...
our daily workweek moods.[fn:23] \\
...
elements denoted by $x$ such that an $x$ can be an element from[fn:28]
$A$ *or* an element from $B$.[fn:29] But wait, *or* doesn't quite
...
together an infinite amount of **singleton**[fn:30] sets of the form
...
numbers $\mathbb{N}$ then what numbers of $\mathbb{N}$[fn:31] can we
...
operation of multiplication is not.[fn:32]
...
usually meant to be **ordered**.[fn:33] Again, idempotency of two
...
[fn:2] An entire field of higher math is devoted to just this sort of
...
[fn:3] More about proofs soon.

[fn:4] A /math holiday/ is a euphemism for taking a detour, going down
a rabbit hole to delve into some math topic necessary to advance the
discussion.

[fn:5] Apostol's /Calculus, Volume 1: One-variable calculus, with an
introduction to linear algebra/ straddles the line between a Calculus
and a much deeper theoretical Analysis book.

[fn:6] Let us be cognizant that Apostol's /Calculus.../ is a text
meant for advanced college math students. But in the real world we
need to build confidence in tackling challenging material and gaining
access to deeper truths.

[fn:7] Content starting with a ⇲ symbol and in blue is what Grok
has told us (with some editing, addendum for clarity). Following in black
is more discussion, explanations.

[fn:8] In math and logic a **vacuous truth** is a statement that is
...
[fn:9] A *[[https://en.wikipedia.org/wiki/Conjecture][conjecture]]* is a tentative *[[https://en.wikipedia.org/wiki/Proposition][proposition]]* that may evolve to
...
[fn:10] For example the (in)famous /[[https://en.wikipedia.org/wiki/Pons_asinorum][pons asinorum]]/ in /Euclid's
Elements/ \\
...
[fn:11] In mathematical logic the idea of /implication/ is very
...
[fn:12] /Higher math/ in the U.S. typically means those college math
...
[fn:13] These axioms relate to Linear Algebra.

[fn:14] ... /quod erat demonstrandum/, meaning "that which was to be
demonstrated".

[fn:15] Go ahead and take a "math holiday" by touring [[http://aleph0.clarku.edu/~djoyce/elements/bookI/cn.html][this page]] by
...
[fn:16] Do /not/ worry about what all this means. In time we will
...
[fn:17] ...if we could, would each program we write technically be a
...
[fn:18] ...containing or including many items.

[fn:19] ...although it may be veiled or only partially explored.

[fn:20] We indicate the cardinality of a set $A$ by surrounding it
with vertical pipes: $|\,A\,|$ This is similar to the idea of a
number's /absolute value/ which uses the same vertical bar symbols.

[fn:21] 𝖟𝕭 is German for /zum Beispiel/ or /for example/.

[fn:22] ...paying double membership fees?

[fn:23]  Notice once again the nested parentheses. To be sure,
everything in Scheme is a list.

[fn:24] A Venn diagram of two sets /or/-ed, /union/-ed together. \\
...
[fn:25] $\cup$ is the symbol for union.

[fn:26] Could we have said, ...in $A$ *and* in $B$ *and* in both $A$
and $B$? Better not. More on why later.

[fn:27] (1) can be expanded for the union of more sets than just
two, e.g., $A \cup B \cup C \cup \ldots$.

[fn:28] The symbol $\in$ means "in" or "member of."

[fn:29] $\lor$ is the symbol for **[[https://en.wikipedia.org/wiki/Logical_disjunction][logical or]]**, aka **logical
disjunction**. Take a crack at the Wikipedia article.

[fn:30] As we saw above, a **singleton** (also known as a **unit set**
or **one-point set**) is a set with exactly one element.

[fn:31] $\mathbb{N}$ is the symbol for the /natural/ numbers, i.e., the
positive integers $1$, $2$, $3\ldots$ we use to count things.

[fn:32] In her 2023 layman's book /The Joy of Abstraction/
...
[fn:33] ...such as, e.g., a pair of /Cartesian coordinates/ $(a,b)$
...
[fn:36] Βιγ Σξηεμερ: Big Schemer; Λιττλε Σξηεμερ: Little Schemer in
Greek letters.

[fn:35] Go [[https://en.wikipedia.org/wiki/Formalism_(philosophy_of_mathematics)][here]] to see the Wikipedia discussion about mathematic
formalism---although it's a bit formal...

[fn:34] Despite containing actual Scheme programs, i.e., real code, TLS
