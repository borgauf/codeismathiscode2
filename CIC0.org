...
ground. Ironically, they know how to get the capsule into orbit, /but
...
descent path[fn:2] back down on the ground. This is a rare moment in a
popular Hollywood film where the filmmakers actually get the science
right. That is to say, there is no magic, no special effects, no
superheroes with super-human powers, no brainiac grade-schooler who
...
cake is how the film depicts the team getting their just-delivered IBM
mainframe computer (that initially no one knows how to run!) to help
them. Takeaway: In the world of STEM you're never far from the front
lines, the cutting edge, the "no one has done this before."

Fast-forward to today where /tens of billions/ of computing
devices---nearly all much more powerful than that first room-sized
NASA computer---are doing computations big and small in every corner
of the world. Of course most of these calculations are relatively
simple, but many are quite complex. This is the sort of math that,
pre-Computer Age, would have been found only in dense books and dry
collections of tables, on classroom and office blackboards, embodied
in mechanical contraptions or analog electrical circuits, and, most
mysteriously, as "mental representations" in the brains of that small
cadre of people called /mathematicians/, /physicists/ and
/engineers/. Now math is happening on computers. We could say an
important part of human intelligence has been /externalize/ to an
inanimate object. And so today we should not underestimate the
...
future. Our mission: see the math, /grok/[fn:3] the math, then get the
...
area of language.[fn:4] When we say, e.g., /It's not that I didn't
...
more numbers. As an exercise he asks the reader to /prove/[fn:6]
...
computers to take in, to /grok/ math axioms and theorems[fn:7] to the point
...
/ChatGPT/, /Gemini/, and /Grok/, no?[fn:8] Here's a good quote from
...
for a layperson to understand.[fn:9] Now, does this mean Grok was
...
/Informatiker/[fn:10] will have to keep up.
...
programming[fn:11]. But still to this day we do not fully understand
...
Contrary to what various "learn to code" promoters[fn:12] might say,
...
want to build on this winning combination[fn:13].
...
a /typed, purely functional/ [fn:14] language. One big reason Haskell
...
mathematical elegance[fn:15]. But why choose Haskell with its rather
...
/declarative/[fn:16] where /code can be considered mathematical
...
predictability.[fn:17] Being able to know and trust your code helps
...
to do[fn:18]. By the way, there's a very big push these days to make
...
So if not Blub, why not a CAS[fn:19], i.e., a math software package
...
Haskell[fn:20]. Toolbox-building, on the other hand, will be more
...
calculation into concepts.[fn:21]

** Math is abstraction

We believe anyone can do math, not because we're cheery, sunny booster
optimists, but because it's just a fact.[fn:22] But why then is math so
...
before[fn:23]. Long story shortened, math can be a very imposing
...
audience[fn:24]. But if we approach the abstraction curve with respect
for its difficulty and steepness /and/ we don't resort to force or
hand-waving,[fn:25] we can master deeper symbolism and abstraction. We
...
forthcoming, rather, just more imitation, conditioned responses...

...and so begins years of teachers drilling students with,
@@html:<font color = "#650d1c">@@When you see this, do
this@@html:</font>@@, what mathematician [[https://longformmath.com/][Jay Cummings]] calls /The
...
understanding of math, let alone its purpose, let alone again its
aesthetics or beauty. Students, like circus animals, simply become
bored and demoralized. By the time a student has reached Algebra I
they only have experience mimicking the teacher, little experience
with actually thinking about math, rather, like the circus beast, only
responding to irksome stimuli.
...
the supporting ideas.[fn:26] This is all about not being afraid to
...
to set the pace and deadlines[fn:31].
...
developed and popularized by the statistician and all-around science
guy [[https://en.wikipedia.org/wiki/Edward_Tufte][Edward Tufte]], best known for his series of books on statistical
information display starting with [[https://www.edwardtufte.com/book/the-visual-display-of-quantitative-information/][The Visual Display of Quantitative
Information]]. This is a modified [[https://orgmode.org/][org-mode]] version[fn:32] of the [[https://edwardtufte.github.io/tufte-css/][Tufte
CSS]], which was developed for producing LaTeX books and websites in the
Tufte style.
...
consumption. As you will see, we make liberal use of this feature.
...
you can dive as deep as you'd like. Be warned, math and computer
science on Wikipedia can go deep, so try not to get too
intimidated. And yet often enough something useful can be gleamed from
a Wiki article.
...
[fn:1] Hold also the though that we /count/ by establishing a
one-to-one /relationship/ with the objects we're counting and the
/natural/ or /counting/ numbers, $1,2,3,\ldots$. Relationships are big
in math.

[fn:2] Technically speaking, a spacecraft orbiting the Earth is in an
/elliptic/, not a perfectly circular orbit. And the path back to Earth
after breaking this orbit will resemble half of a parabola turned on
its side, hence, the adjective parabolic.

[fn:3] To **grok** something is to understand it at its deepest level,
...
[fn:4] If we said $a = a$ and $\lnot\; a = \lnot\; a\;$ and $\lnot\;a
\ne a$ and that $\lnot\; \lnot\;a = a\;$ what can we infer the $\lnot$
symbol is doing? More later.

[fn:5] *[[https://en.wikipedia.org/wiki/Prolog][Prolog]]* is a programming language in which rules, facts, and
relational /predicates/ are given, then logic /deductions/ can be
made. More about all this upcoming.

[fn:6] ...below, the tuple form $(a,b)$ means /the greatest common
divisor of $a$ and $b$./

[fn:7] We'll explore **[[https://en.wikipedia.org/wiki/Axiomatic_system][axiomatic mathematics]]** in great detail as we
...
[fn:8] By the way, [[file:gcdproof1.html][here]] is a math proof generated by Google's
Gemini. Ironically, we've essentially asked something not
logically-based to do, simulate logic...

[fn:9] See [[file:primesUpTo.html][primesUpTo]], the Grok answer converted into a plain org
file.

[fn:10] /Informatik/ is the German term for /computer science/. Edsger
...
[fn:11] There are two general categories of "giving instructions" to a
...
[fn:12] Most "learn to code" initiatives are rushed oversimplifications
...
[fn:13] One huge inspiration for CIMIC is the book /[[https://en.wikipedia.org/wiki/Structure_and_Interpretation_of_Classical_Mechanics][Structure and
...
[fn:14] We'll dive into what /typed/ and /functional/ languages are
later...

[fn:15] Often enough, you can open a higher-level math text, e.g.,
...
[fn:16] Maybe peruse [[https://en.wikipedia.org/wiki/Declarative_programming][this article]] on declarative programming. Don't
worry, we'll cover all this in depth as we go.

[fn:17] We'll discuss no side effects or /referential transparency/,
...
[fn:18] It might be hard to get all of what Charles Scalfani is saying
...
[fn:19] CAS is short for /computer algebra system/. Wikipedia has a good
article [[https://en.wikipedia.org/wiki/Computer_algebra_system][here]].

[fn:20] ...as well as its very worthy siblings in the typed functional
sphere: SML, Ocaml, Purescript, and F#.

[fn:21] Perhaps watch [[https://youtu.be/6APBx0WsgeQ?si=Hpl1zLcdjkuDtHDP][Why does Cambridge teach OCaml as the first
programming language?]] to better understand why functional.

[fn:22] Watch [[https://youtu.be/-MTRxRO5SRA?si=3nvDTCra6MV2rJTj][Let's teach for mastery, not test scores; Sal Khan]]. A
real game-changer.

[fn:23] See [[https://youtu.be/9vpqilhW9uI][James Flynn]]'s TED talk, and read the first chapter of James
...
[fn:24] The classic line, "What is this for? Why are we doing this?"
is the elephant in the middle of the math classroom.

[fn:25] To /hand-wave/ in science is to dismiss or downplay
...
[fn:32] Org-mode is a text management system app of the mighty Emacs
text editing world.
