...
theorems, even if they have only been presented as a big, amorphous
tool bag for solving problems.[fn:8] The axiom-to-theorems approach is
diametrically opposed to typical K-12 "muscle-memory" math where the
mantra is "Do enough problems and you'll catch on."

Tom Apostol in his /[[https://en.wikipedia.org/wiki/Calculus_(Apostol_books)][Calculus...]]/[fn:9] textbook starts out with just
...
In higher math the concept of equality, of sameness goes deeper and is
more precise and nuanced than our intuitive understanding. Formally,
equality is an /equivalence relation/ on numbers. We'll look into
exactly what a relation is in the higher math sense soon. Grok's
answer is couched in terms of higher math, which is based on **[[https://en.wikipedia.org/wiki/Set_theory][set
...
thorough and foundational basis of math, upon which all math could be
axiomatically built. We will weave both set theory and logic into our
presentations as we go. One part of logic is **[[https://en.wikipedia.org/wiki/First-order_logic][first-order logic]]** or
FOL. Again, nothing in depth now, but we'll need to get a basic
understanding to understand what's being said in the /Substitution
Principle/ above.

In this first section of Grok's answer we see rules of equality
similar to the previous discussion. First is **reflexivity**, which
might seem vacuous and unnecessary to the layperson. Of course any
given thing is the same, /equal/ to itself. But consider this:  a relation each thing
bears to itself and nothing else But we have to state this explicitly
to be thorough. Next, **symmetry** is allowing expressions to swap
sides of an equation, i.e., what's on the left can go to the right and
vice-versa, and they're still equal. Again, you were just conditioned
...
axioms[fn:11] ... and EE has been, in one form or another, a part of
...
situation[fn:12] and understand why something about it is /necessarily/
...
givens.[fn:13] For example, the last notion, N5, seems to say something
...
useful math is solidly, logically built. Higher math[fn:14] is the
...
i.e., relying on these axioms[fn:15]
...
a set of tools.[fn:17]

** Axiomatic Scheme

And now with no further ado we will list out TLS's version of
"axiomatic givens" starting with /The Ten Commandments/ below[fn:18]
...
exactly how to write a Scheme program.[fn:19] But then if you've read
...
omnibus[fn:20] /list processing/ machinery. In fact Scheme's
...
higher math, as well as CS's Discrete Math.[fn:21] And so many books
...
elements of the set, /not/ counting duplicates.[fn:22]
...
/belong-belong/ or belong twice to the club.[fn:24] In the example
...
our daily workweek moods.[fn:25] \\
...
elements denoted by $x$ such that an $x$ can be an element from[fn:30]
$A$ *or* an element from $B$.[fn:31] But wait, *or* doesn't quite
...
together an infinite amount of **singleton**[fn:32] sets of the form
...
numbers $\mathbb{N}$ then what numbers of $\mathbb{N}$[fn:33] can we
...
operation of multiplication is not.[fn:34]
...
usually meant to be **ordered**.[fn:35] Again, idempotency of two
...
[fn:8] ...although the so-called Common Core curriculum initiatives
...
[fn:9] Apostol's /Calculus, Volume 1: One-variable calculus, with an
introduction to linear algebra/ straddles the line between a Calculus
and a much deeper theoretical Analysis book.

[fn:10] Content starting with a ⇲ symbol and in blue is what Grok
has told us (with some editing, addendum for clarity). Following in black
is more discussion, explanations.

[fn:11] A *[[https://en.wikipedia.org/wiki/Conjecture][conjecture]]* is a tentative *[[https://en.wikipedia.org/wiki/Proposition][proposition]]* that may evolve to
...
[fn:12] For example the (in)famous /[[https://en.wikipedia.org/wiki/Pons_asinorum][pons asinorum]]/ in /Euclid's
Elements/ \\
...
[fn:13] In mathematical logic the idea of /implication/ is very
...
[fn:14] /Higher math/ in the U.S. typically means those college math
...
[fn:15] These axioms relate to Linear Algebra.

[fn:16] ... /quod erat demonstrandum/, meaning "that which was to be
demonstrated".

[fn:17] Go ahead and take a "math holiday" by touring [[http://aleph0.clarku.edu/~djoyce/elements/bookI/cn.html][this page]] by
...
[fn:18] Do /not/ worry about what all this means. In time we will
...
[fn:19] ...if we could, would each program we write technically be a
...
[fn:20] ...containing or including many items.

[fn:21] ...although it may be veiled or only partially explored.
...
[fn:23] 𝖟𝕭 is German for /zum Beispiel/ or /for example/.

[fn:24] ...paying double membership fees?

[fn:25]  Notice once again the nested parentheses. To be sure,
everything in Scheme is a list.

[fn:26] A Venn diagram of two sets /or/-ed, /union/-ed together. \\
...
[fn:27] $\cup$ is the symbol for union.

[fn:28] Could we have said, ...in $A$ *and* in $B$ *and* in both $A$
and $B$? Better not. More on why later.

[fn:29] (1) can be expanded for the union of more sets than just
two, e.g., $A \cup B \cup C \cup \ldots$.

[fn:30] The symbol $\in$ means "in" or "member of."

[fn:31] $\lor$ is the symbol for **[[https://en.wikipedia.org/wiki/Logical_disjunction][logical or]]**, aka **logical
disjunction**. Take a crack at the Wikipedia article.

[fn:32] As we saw above, a **singleton** (also known as a **unit set**
or **one-point set**) is a set with exactly one element.

[fn:33] $\mathbb{N}$ is the symbol for the /natural/ numbers, i.e., the
positive integers $1$, $2$, $3\ldots$ we use to count things.

[fn:34] In her 2023 layman's book /The Joy of Abstraction/
...
[fn:35] ...such as, e.g., a pair of /Cartesian coordinates/ $(a,b)$
