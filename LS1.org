...
page of the first chapter entitled /1. Toys/![fn:1]
...
| ~(atom turkey or)~                 |                                                                                                                                             |
|                                    |                                                                                                                                             |
This Socratic dialog in its chatty conversation style is teasing out
the requisite /mathematical formalism/ [fn:2] where you the reader
will experience a growing mental picture of theory and syntax as the
conversation continues and the pages turn. The idea is the formalisms
were always there, and we will gradually /coax/ them out with
carefully constructed prodding. Instead of making direct statements
about programming language theory or syntax rules, we get a stream of
questions and replies that will form the desired world in our own
minds. We are if effect /backing into/ Scheme programming and its
underlying philosophy---and underlying is quite a bit of important
computer science.[fn:3]
...
the student is asked to see a geometric situation[fn:4] and understand
...
mood data we're collecting each day of week 3.[fn:16] \\
⌟
...
can be shown with a /[[https://en.wikipedia.org/wiki/Union_(set_theory)][union]]/ of circles.[fn:17]
...
/both/ $X$ and $Y$ ---and the other way around as
well. $\mathfrak{Fazit}$: We avoid writing $X \lor Y$ when considering
just the set without involving the elements therein.
...
[fn:3] Despite containing actual Scheme programs, actual code, TLS was
written so that a layperson could follow along with no knowledge of
computers, e.g., someone from the eighteenth century!
...
[fn:7] These axioms are taken from Linear Algebra.
...
[fn:13] We indicate the cardinality of a set $A$ by
...
[fn:16] With our first introduction to Scheme syntax we've made a list
