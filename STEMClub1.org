...
<<6cf24bbe-c37f-4480-8b57-79509a9adc40>>
...
devices---and putting in into digital computing devices.

** Basic fractions on the computer

Let's go down the rabbit hole of how to do fractions---specifically,
/adding, subtracting unlike denominator fractions/---on the
computer. Here we enter The world of number theory and its
concentration on the behavior of prime numbers.

@@html:<font color = "#4715b3">@@ ⥤ A *prime number* is an integer
that can only be divided by $1$ or itself.  @@html:</font>@@ \\
\\
A prime number is /not/ a *composite number*, i.e., a multiple of
other numbers. This means there is no number $d$ between $1$ and the
prime $p$ that will evenly divide it.

⇲ Theorem *Fundamental Theorem of Arithmetic*

@@html:<font color = "#4715b3">@@Every positive integer $𝑛 \gt 1$ can be
...
$\mathbb{N}$, by simply adding "one more" over and over.[fn:3] But we
can also "get" any $n \in \mathbb{N}$ by multiplying sets of
primes. Hence, we can almost say, prime numbers /necessitate/ (bring
into existence?) multiplication. At the very least primes beg their multiplication.
...
given any specific $a$ or $b$.[fn:6] Another identity would be
...
mathematically, although they're obviously different.[fn:7] /Which
...
with the Haskell REPL command ~:info~ (or just ~:i~)[fn:8]
...
Now, back to ~divides d n = rem n d == 0~. Which is really just a nice
/is-defined-as/ wrapper for the /identity/ ~rem n d == 0~, i.e., we
have created a Haskell version of $d \mid n$. Let's look at its type
...
Here Haskell reports ~divides~ takes two arguments of type ~a~,
...
$sin^{2}\theta + cos^{2}\theta$, is whatever $\theta$ we plug in, and the value of the
...
=>~ indicates whatever actual types ~a~ is, those types must be
...
As the ellipsis (...) indicate, much was cut out for brevity. But of
course the idea behind ~Eq~ is mathematical logic equality or
*equivalence*[fn:9]

⥤ *Side note* establishing equivalence \\

It may be obvious that $1$ is equivalent to $1$, but what if we have
created our own data type, say
...
...meaning our ~Color~ type does not have addition defined. The error
message indicates the type class ~Num~ doesn't have an /instance/ of
~Color~ registered with it. Can we /compare/ colors for order or
equivalence? Not yet. But if we specifically create an instance in
~Eq~ telling Haskell what is, e.g., equivalent
...
So we have a Haskell code version of math's $d$ /divides $n$. Let's
move on.
...
Every number has at least one divisor.[fn:10] Therefore, every number
$n$ has a least divisor, $LD(n)$, of its set of divisors, even if that
set has only one element. For example $12$, a composite number, has
the set of divisors $\{2,3,4,6,12\}$ with $LD(12) = 2$. But what if
the set of $LD(n)$ is just one number? What sort of number would $n$
therefore be? Intuitively we know that if a number has only one
divisor, it cannot be any sort of composite number; otherwise, the
number would still have a divisor. But that's just our intuition...

Consider the set of divisors of $12$ again: $\{2,3,4,6,12\}$. We see
two primes to start with, $2$ and $3$---then non-prime composites,
$4$, $6$, $12$. The least divisor in this set is $2$, and it is
prime. What about $120$? The divisor set[fn:11] would be

\begin{align*}
\{2,3,4,5,6,8,10,12,15,20,24,30,40,60,120\}
\end{align*}

and we can get it with this Haskell code
...
Again, we see a pattern where we start with divisors that are prime
numbers, then (if any) composite numbers appear.[fn:12] In both
examples notice that the square the $LD(n)$, e.g., with $120$ squaring
$2$ is $4$, i.e., less than $120$. Why is this germane?

Consider the composite number $143$. Its divisors are two prime
numbers, $11$ and $13$. Now, $11^2 = 121$, while $13^2 = 169$, i.e.,
the square of the former is less than $143$ and the square of the
latter is greater than $143$. Neat observation, but does it help us
ultimately in testing a number for prime-ness? Yes, and we'll find out
why below.
...
Why concentrate on the least divisor? The first part of the
...
primeness and no divisor less than the number itself is found. We need
to be sure that least divisor is indeed prime.

Besides just our intuitive notion, we can prove this. We'll employ
/contradiction/[fn:13], i.e., we set up a contradiction to what we
want to prove, then exposing it as not true. In this case, let's
assume $c = LD(n)$ and $c$ is /not/ prime, a contradiction of our
original statement. But if $c$ is not prime it must then be a
composite number itself, e.g., $c = a \cdot b$ where $1 \lt a \lt c$. But that
would mean $a$ now divides $n$, and $a$ is smaller than $c$ the
supposed $LD(n)$. Therefore, a $LD(n)$ must be prime.
...
because if both $d$ and $e$ were greater than $\sqrt{599}$ that would
make $d \cdot e$ greater than $\sqrt{599} \cdot \sqrt{599} = 599$, which it
can't be. So if $599$ is composite, it has a factor between $2$ and $\lfloor
\sqrt{599} \rfloor$[fn:14] or $24$.

At this point we can guess that to show $599$ is prime, we need to
show it is not a composite number. $24^2 = 576$, the /floor/ of
...
before we get to $599$. If we test this on $599$ we see that none of
the primes between $2$ and $24$, namely the set
$\{2,3,5,7,11,13,17,19,23\}$ divides $599$ evenly; therefore, $599$ is
prime.

So we have our main tool for establishing primality, namely,
$LD(n)$. Let's put it into code. We'll break it into two parts
...
"least divisor function." What we see with ~ldf~ is Haskell's
*equation guarding*, which allows us to set up test *cases*. ~ldf~
first tries ~divides~ (which yields either ~True~ or ~False~) on the
incoming number argument ~n~ using the argument ~k~. We see ~ldf~
begins with the lowest prime $2$ for ~k~. So if $2 \mid n$ then we're
through, the $LD(n) = k$. But if it is not then we move on to the next
/case/, namely, Is ~k~ squared greater than ~n~? Again, this yields a
~True~ or ~False~. If ~True~, then ~n~ is the least divisor. But if
this second test case also fails, we go onto the third, which is not a
normal test case, but a hardwired ~True~, i.e., we're going with this
case---because there are no other possible situations. What's unique
about this last case is that it simply calls itself, but with ~k~
incremented by $1$. In other words, we have a repeating mechanism
whereby we can march up through the whole numbers one after the other
testing each. This is *recursion*, and we'll soon be exploring it in
depth. But for now just accept that ~ldf~ is calling itself, driving
inside itself, so to speak, to try another time.
...
is also ~False~. Hence, recursion ensues with ~ldf 3 5~. On the first
test case ~divides 3 5~ also fails, but then the second test case,
~3^2 > 5~ is ~True~; and so we know $5$ is indeed prime. And so we
have turned our Proposition theorems into an algorithm and working
code! Let's see a more general (non-abbreviated) explanation of
/equation guarding/
...
So now let's define our first version of a /test for primes/
...
provide a type signature for ~prime0~ and then suggests one
...
[fn:5] In higher math the *natural counting numbers*
$\{1,2,3,4,\ldots\}$ are signified by $\mathbb{N}$, while the
*integers* $\{\ldots,-2,-1,0,1,2,\ldots\}$ are $\mathbb{Z}$. Both are
whole numbers.

[fn:6] In Haskell, everything has a value. This is an abstraction
...
[fn:7] The [[https://en.wikipedia.org/wiki/Pythagorean_trigonometric_identity][proof/derivation]] of $sin^{2}\theta + cos^{2}\theta = 1$ is usually included
...
[fn:8] ~:t~ and ~:i~ are your best friends when navigating the Haskell
type world.

[fn:9] In mathematical logic statements, expressions $p$ and $q$ are
*[[https://en.wikipedia.org/wiki/Logical_equivalence][logically equivalent]]* if they have the same /truth value/ in every
case of possible inputs. Moral of the story: Keep /is-defined-as/ ~=~
separate from /equivalence/ ~==~ (or non-equivalence ~/+~).

[fn:10] A prime number has exactly one divisor greater than $1$, i.e.,
the prime number itself. Remember this for later...

[fn:11] ...or the /factors/ of $120$.

[fn:12] Let's not forget the /Fundamental Theorem of Arithmetic/,
i.e., that $120$ can also be expressed as product of powers of those
first three primes.

[fn:13] See [[https://en.wikipedia.org/wiki/Proof_by_contradiction][proof by contradiction]].

[fn:14] The *floor* ($\lfloor\; \rfloor$) of $\sqrt{599}$? First, get the square
