...
devices---and putting in into digital computing devices. And with the
digital computing devices count about to exceed /three times/ the
world's human population, that's a big deal.
...
$\mathbb{N}$,[fn:3] by simply adding "one more" over and over.[fn:4]
...
were entered into a Gemini search[fn:5]
...
they're obviously different symbols.[fn:7] /Which means they are
...
behind ~Eq~ is mathematical logic equality or *equivalence*[fn:9]
...
...meaning our ~Color~ type does not have addition defined.[fn:10] The
...
Every number has at least one divisor.[fn:11] Therefore, every number
...
prime. What about $120$? The divisor set[fn:12] would be
...
numbers, then (if any) composite numbers appear.[fn:13] In both
...
/contradiction/[fn:14], i.e., we set up a contradiction to what we
...
set $\{2,3,5,7,11,13,17,19,23\}$ divides $599$ evenly; therefore,
$599$ is prime.
...
/case/, namely, Is ~k~ squared greater than ~n~?[fn:16] Again, this
yields a ~True~ or ~False~. If ~True~, then ~n~ is the least
divisor. But if this second test case also fails, we go onto the
third, which is not a normal test case, but a hardwired ~True~, i.e.,
we're going with this case---because there are no other possible
situations. What's unique about this last case is that it simply calls
itself, but with ~k~ incremented by $1$. In other words, we have a
repeating mechanism whereby we can march up through the whole numbers
one after the other testing each. This is *recursion*, and we'll soon
be exploring it in depth. But for now just accept that ~ldf~ is
calling itself, driving inside itself, so to speak, to try another
time.
...
math is just out there in the universe, later.
...
[fn:3] In higher math the *natural counting numbers*
$\{1,2,3,4,\ldots\}$ are signified by $\mathbb{N}$, while the
*integers* $\{\ldots,-2,-1,0,1,2,\ldots\}$ are $\mathbb{Z}$. Both are
whole numbers.

[fn:4] Fundamental to Peano's approach is recursion and induction,
which we will explore in depth soon.

[fn:5] This is not serious number factoring code. The "art" of
...
[fn:10] Technically only numbers can be added, subtracted,
etc. For addition, e.g., non-number objects are /aggregated/.

[fn:11] A prime number has exactly one divisor greater than $1$, i.e.,
the prime number itself. Remember this for later...

[fn:12] ...or the /factors/ of $120$.

[fn:13] Let's not forget the /Fundamental Theorem of Arithmetic/,
i.e., that $120$ can also be expressed as product of powers of those
first three primes.

[fn:14] See [[https://en.wikipedia.org/wiki/Proof_by_contradiction][proof by contradiction]].

[fn:15] The *floor* ($\lfloor\; \rfloor$) of $\sqrt{599}$? First, get the square
...
[fn:16] This is the second part of $LD(n)$ Proposition, i.e., we've
