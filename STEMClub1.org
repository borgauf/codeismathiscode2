...
Every number has at least one divisor greater than /unit/ $1$.[fn:11]
...
$\{2,3,4,6,12\}$. Here, $LD(12) = 2$ and the *cardinality*[fn:12] of
the set is $5$, i.e., greater than $1$. What about $120$? The divisor
set[fn:13] would be much larger

\begin{align*}
\{2,3,4,5,6,8,10,12,15,20,24,30,40,60,120\}
\end{align*}

and we can get it with this Haskell code[fn:14]
...
prime, then (if any) composite numbers follow.[fn:15] In both examples
...
We'll employ a proof method called /contradiction/[fn:16] where we
...
is composite, it has a divisor between $2$ and $\lfloor \sqrt{599} \rfloor$[fn:17]
or $24$.
...
~n~?[fn:18] Again, this yields a ~True~ or ~False~. If ~True~, then
...
[fn:11] Again, a prime number has exactly one divisor greater than $1$, i.e.,
the prime number itself.

[fn:12] The *cardinality* of a set is the count of the unique elements
contained in the set.

[fn:13] ...or the /factors/ of $120$.

[fn:14] This is the Haskell version of *[[https://en.wikipedia.org/wiki/Set-builder_notation][set comprehension]]*. More
later.

[fn:15] Let's not forget the /Fundamental Theorem of Arithmetic/,
i.e., that $120$ can also be expressed as product of powers of those
first three primes.

[fn:16] See [[https://en.wikipedia.org/wiki/Proof_by_contradiction][proof by contradiction]].

[fn:17] The *floor* ($\lfloor\; \rfloor$) of $\sqrt{599}$? First, get the square
...
[fn:18] This is the second part of $LD(n)$ Proposition, i.e., we've
