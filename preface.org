...
/[[https://en.wikipedia.org/wiki/Generative_artificial_intelligence][Generative artificial intelligence]]/ is the latest trend in the race
towards AI. Today we can describe a problem to an AI bot---and presto!
it can seemingly magically give us a response. For example, we ask
[[https://grok.com/][Grok]]
...
for a Haskell beginner to understand.[fn:3] Now, does this mean all
computer programmers are out of a job? Of course not. What it really
means is /now more than ever you need to have great computer science
knowledge/. Why? Because you need to

- know what to ask
- know how to ask it
- understand AI's response
- be able to apply the code
- take responsibility of the code as a quasi-manager
...
/Informatiker/[fn:4] will have to keep up.
...
programming[fn:5]. But still to this day we do not fully understand
...
Contrary to what various "learn to code" promoters[fn:6] might say,
...
and we want to build on this winning combination[fn:7].
...
a /typed, purely functional/ [fn:8] language. One big reason Haskell
...
mathematical elegance[fn:9]. But why choose Haskell with its rather
...
do.[fn:10] This is called /imperative/ programming, i.e., each line is
...
/declarative/[fn:11] languages where code can be considered
...
/state/ of the program has changed.[fn:12] This might seem too
...
absolute necessity for computational predictability.[fn:13] Being able
...
than what you expected it to do[fn:14]. By the way, there's a very big
...
So if not Blub, why not a CAS[fn:15], i.e., a math software package
...
Haskell[fn:16]. Toolbox-building, on the other hand, will be more
...
calculation into concepts.[fn:17]
...
before[fn:18]. Long story shortened, math can be a very imposing
...
audience[fn:19]. But if we approach the abstraction curve with respect
for its difficulty and steepness /and/ we don't resort to force or
hand-waving[fn:20], we can master deeper symbolism and abstraction. We
...
/discrete/ mathematics[fn:22]. Without any previous exposure to the
...
march to catch up[fn:23]. The result is, nation-wide, high
drop-out/flunk-out rates in CS departments...

...but not necessarily at the elite universities with their
90th-percentile students[fn:24]. At the world-class STEM schools the
...
algorithms[fn:25], data structures, and numerical applications
...
/rabbit-holing/[fn:26] from now on. We're all about /[[https://en.wikipedia.org/wiki/Depth-first_search][depth-first]]/
learning[fn:27], not /[[https://en.wikipedia.org/wiki/Breadth-first_search][breadth-first]]/. Typically, breadth-firsters want
...
and that, do some rabbit-holing and woodshedding[fn:28], ask around if
...
set the pace and deadlines[fn:29].
...
operating system[fn:30]. Sure, you can use Mac or Windows, but we'll
...
We'll also use the /Emacs/ text editor. [[https://en.wikipedia.org/wiki/Emacs][Emacs]][fn:31] is far more than
...
deducible or LEs of the explicitly given facts[fn:32].
...
face in their first year in a Computer Science program[fn:33].
...
[fn:3] See [[file:primesUpTo.html][primesUpTo]], the Grok answer converted into a plain org
file.

[fn:4] /Informatik/ is the German term for /computer science/. Edsger
...
[fn:5] There are two general categories of "giving instructions":
/imperative/ with statements and /declarative/ with expressions. Read
on.

[fn:6] Most "learn to code" initiatives are rushed oversimplifications
...
[fn:7] One huge inspiration for CIMIC is the book /[[https://en.wikipedia.org/wiki/Structure_and_Interpretation_of_Classical_Mechanics][Structure and
...
[fn:8] We'll dive into what /typed/ and /functional/ languages are
later...

[fn:9] Often enough, you can open a higher-level math text, e.g.,
...
[fn:10] /Literally/ line-for-line programming would be [[https://en.wikipedia.org/wiki/Assembly_language][Assembly]]
...
[fn:11] See [[https://en.wikipedia.org/wiki/Declarative_programming][this article]] on declarative programming.

[fn:12] When did you first hear about a true function having only one
...
[fn:13] We'll discuss no side effects or /referential transparency/,
...
[fn:14] It might be hard to get all of what Charles Scalfani is saying
...
[fn:15] CAS is short for /computer algebra system/. Wikipedia has a good
article [[https://en.wikipedia.org/wiki/Computer_algebra_system][here]].

[fn:16] ...except for its very worthy siblings in the typed functional
sphere SML, Ocaml, Purescript, and F#.

[fn:17] Perhaps watch [[https://youtu.be/6APBx0WsgeQ?si=Hpl1zLcdjkuDtHDP][Why does Cambridge teach OCaml as the first
programming language?]] to better understand why functional.

[fn:18] See [[https://youtu.be/9vpqilhW9uI][James Flynn]]'s TED talk, and read the first chapter of James
...
[fn:19] The classic line, "What is this for? Why are we doing this?"
is the elephant in the middle of the math classroom.

[fn:20] To /hand-wave/ in science is to dismiss or downplay
complexity, to skip over or sweep something difficult under the
rug. We will avoid hand-waving at all costs.

[fn:21] Watch Sabine Hossenfelder's video on the [[https://youtu.be/A0da8TEeaeE][Principle of Least
Action]]. It's a bit beyond our level, but you get the "unpack, trace it
back" idea.

[fn:22] We'll learn plenty about discrete math as we go. But for a
...
[fn:23] Imagine wanting to study math in college after having seen
...
[fn:24] ...as I refer to them the /big boys and girls/ who can do the
"heavy lifting"...

[fn:25] You've no doubt heard of /algorithms/. Very simply put, an
...
[fn:26] "Down the rabbit hole" is a concept borrowed from /Alice in
Wonderland/ to mean going into a strange, new world of excitement and
challenge.

[fn:27] But of course we run the risk of "getting lost in the weeds."
...
[fn:28] /Woodshedding/ is a jazz music term meaning to learn and
practice.

[fn:29] One motivational speaker said there are two kinds of
...
[fn:30] See /Rig Rundown/ for hardware options.

[fn:31] See also [[https://www.gnu.org/software/emacs/][the home site]] and this [[https://planet.emacslife.com/][user site]].

[fn:32] Here's an example of LE. If we said $\lnot\; a = \lnot\; a\;$
and that $\lnot\; \lnot\;a = a\;$ what can we infer that $\lnot$ is
doing to $a\;$?

[fn:33] If anyone is bothered about why we're not being more woke/PC,
