...
get a space capsule flying in orbit around the Earth back on the ground. They
...
--- nearly all many, many times more powerful than that first NASA
computer --- are doing computations big and small in every corner of
the world. Of course most of these calculations are relatively simple,
but many are quite complex. This is the sort of math that,
...
/engineers/. Now math is in computers.
...
programming[fn:3]. But still to this day we do not fully understand
...
do.[fn:8] This is called /imperative/ programming, i.e., each line is
...
/state/ of the program has changed.[fn:10] This might seem too
...
it to do[fn:12]. By the way, there's a very big push these days to
make code "provable," i.e., guaranteed to do what it's supposed to and
not what it's not supposed to do. Typed functional languages like
Haskell are the base camp of this mountain.
...
So if not Blub, why not a CAS[fn:13], i.e., a math software package
...
knife (C++ with a steak knife). Haskell---for many reasons---is
like eating soup with a spoon. It's the best utensil.
...
just how and why Haskell is doing something the way it does, i.e.,
based on math. Again, there's no other language like
Haskell[fn:14]. Toolbox-building, on the other hand, will be more
"learn to code," only with Haskell, i.e., figuring out what its
syntax, semantics, its /first principles/ hands-on. So yes, with
Haskell we'll be building and designing our own calculator, our own
CAS, but we'll go beyond calculation and into concepts.
...
before[fn:15]. Long story shortened, math can be a very imposing
vertical wall of abstraction. And, as Luria discovered, there may
exist social and cultural barriers, actual social-psychological
resistance /against/ abstraction that we might imbue from our
milieu. We cannot simply assume that any and all abstraction and
symbolism can and will be instantly grokked by our math
audience[fn:16]. But if we approach the abstraction curve with respect
for its difficulty and steepness /and/ we don't resort to force or
hand-waving[fn:17], we can master deeper symbolism and abstraction. We
...
up as many The Ways as possible rarely leads to any real understanding
of math, let alone its purpose, let alone its aesthetics or beauty.
...
account the history, the people behind the supporting ideas[fn:18].
...
/discrete/ mathematics[fn:19]. Without any previous exposure to the
...
march to catch up[fn:20]. The result is, nation-wide, high
drop-out/flunk-out rates in CS departments...

...but not necessarily at the elite universities with their
90th-percentile students[fn:21]. At the world-class STEM schools the
...
algorithms[fn:22], data structures, and numerical applications
...
/rabbit-holing/[fn:23] from now on. We're all about /[[https://en.wikipedia.org/wiki/Depth-first_search][depth-first]]/
learning[fn:24], not /[[https://en.wikipedia.org/wiki/Breadth-first_search][breadth-first]]/. Typically, breadth-firsters want
...
and that, do some rabbit-holing and woodshedding[fn:25], ask around if
...
set the pace and deadlines[fn:26].
...
operating system[fn:27]. Sure, you can use Mac or Windows, but we'll
...
We'll also use the /Emacs/ text editor. [[https://en.wikipedia.org/wiki/Emacs][Emacs]][fn:28] is far more than
...
deducible or LEs of the explicitly given facts[fn:29].
...
face in their first year in a Computer Science program[fn:30].
...
abstract algebra, pick a topic, then Google Haskell and your obscure abstract
algebra topic and, chances are, somebody has explored a
code-is-math-is-code treatment of it, written libraries for it.

[fn:8] /Literally/ line-for-line programming would be [[https://en.wikipedia.org/wiki/Assembly_language][Assembly]]
...
[fn:10] When did you first hear about a true function having only one
...
[fn:13] CAS is short for /computer algebra system/. Wikipedia has a good
article [[https://en.wikipedia.org/wiki/Computer_algebra_system][here]].

[fn:14] ...except for its very worthy siblings in the typed functional
sphere SML, Ocaml, Purescript, and F#.

[fn:15] See [[https://youtu.be/9vpqilhW9uI][James Flynn]]'s TED talk, and read the first chapter of James
...
[fn:16] The classic line, "What is this for? Why are we doing this?"
is the elephant in the middle of the math classroom.

[fn:17] To /hand-wave/ in science is to dismiss or downplay
complexity, to skip over or sweep something difficult under the
rug. We will avoid hand-waving at all costs.

[fn:18] Watch Sabine Hossenfelder's video on the [[https://youtu.be/A0da8TEeaeE][Principle of Least
Action]]. It's a bit beyond our level, but you get the "unpack, trace it
back" idea.

[fn:19] We'll learn plenty about discrete math as we go. But for a
...
[fn:20] Imagine wanting to study math in college after having seen
...
[fn:21] ...as I refer to them the /big boys and girls/ who can do the
"heavy lifting"...

[fn:22] You've no doubt heard of /algorithms/. Very simply put, an
...
[fn:23] "Down the rabbit hole" is a concept borrowed from /Alice in
Wonderland/ to mean going into a strange, new world of excitement and
challenge.

[fn:24] But of course we run the risk of "getting lost in the weeds."
...
[fn:25] /Woodshedding/ is a jazz music term meaning to learn and
practice.

[fn:26] One motivational speaker said there are two kinds of
...
[fn:27] See /Rig Rundown/ for hardware options.

[fn:28] See also [[https://www.gnu.org/software/emacs/][the home site]] and this [[https://planet.emacslife.com/][user site]].

[fn:29] Here's an example of LE. If we said $\lnot\; a = \lnot\; a\;$
and that $\lnot\; \lnot\;a = a\;$ what can we infer that $\lnot$ is
doing to $a\;$?

[fn:30] If anyone is bothered about why we're not being more woke/PC,
